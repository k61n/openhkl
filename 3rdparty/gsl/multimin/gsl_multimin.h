...
#define GSL_MULTIMIN_FN_EVAL_F_DF(F, x, y, g)                                  \
  (*((F)->fdf))(x, (F)->params, (y), (g))

int gsl_multimin_diff(const gsl_multimin_function *f, const gsl_vector *x,
                      gsl_vector *g);
...
typedef struct {
  ...
  int (*iterate)(void *state, gsl_multimin_function *f, gsl_vector *x,
...
typedef struct {
  ...
  int (*iterate)(void *state, gsl_multimin_function_fdf *fdf, gsl_vector *x,
                 double *f, gsl_vector *gradient, gsl_vector *dx);
...
GSL_VAR const gsl_multimin_fdfminimizer_type
