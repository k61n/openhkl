...
int test_sf_check_result(char *message_buff, gsl_sf_result r, double val,
                         double tol);
int test_sf_check_val(char *message_buff, double rval, double val, double tol);
int test_sf_check_return(char *message_buff, int val_return,
                         int expected_return);
int test_sf_check_result_relax(char *message_buff, gsl_sf_result r, double val,
                               double tol);
...
int test_sf_e10(gsl_sf_result_e10 r, double val_in, int e10_in, double tol,
...
int test_sf_rlx(gsl_sf_result r, double val_in, double tol, int status,
                int expect_return, const char *desc);
int test_sf_2(gsl_sf_result r1, double val1, double tol1, gsl_sf_result r2,
              double val2, double tol2, int status, int expect_return,
              const char *desc);
int test_sf_sgn(gsl_sf_result r, double sgn, double val_in, double tol,
                double expect_sgn, int status, int expect_return,
                const char *desc);
...
#define TEST_SF_E10(stat, func, args, val_in, e10_in, tol, expect_return)      \
  {                                                                            \
    int status = func args;                                                    \
    stat += test_sf_e10(re, val_in, e10_in, tol, status, expect_return,        \
                        #func #args);                                          \
  }

#define TEST_SF_VAL(stat, func, args, val_in, tol)                             \
  {                                                                            \
    double val = func args;                                                    \
    stat += test_sf_val(val, val_in, tol, #func #args);                        \
  }

#define TEST_SF_RLX(stat, func, args, val_in, tol, expect_return)              \
  {                                                                            \
    int status = func args;                                                    \
    stat += test_sf_rlx(r, val_in, tol, status, expect_return, #func #args);   \
  }

#define TEST_SF_2(stat, func, args, val1, tol1, val2, tol2, expect_return)     \
  {                                                                            \
    int status = func args;                                                    \
    stat += test_sf_2(r1, val1, tol1, r2, val2, tol2, status, expect_return,   \
                      #func #args);                                            \
  }
...
#define TEST_SF_THETA(stat, func, args, val_in, tol)                           \
  {                                                                            \
    ...
    theta = args;                                                              \
    status = func(&theta);                                                     \
    stat += test_sf_val(theta, val_in, tol, #func #args);                      \
  }
