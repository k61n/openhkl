...
/* cblas_xgemv() */
#define CBLAS_ERROR_GEMV(pos, order, TransA, M, N, alpha, A, lda, X, incX,     \
                         beta, Y, incY)                                        \
...
/* cblas_xgbmv() */
#define CBLAS_ERROR_GBMV(pos, order, TransA, M, N, KL, KU, alpha, A, lda, X,   \
                         incX, beta, Y, incY)                                  \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_TRANSPOSE(pos, 2, TransA);                                             \
  CHECK_DIM(pos, 3, M);                                                        \
...
/* cblas_xtbmv() */
#define CBLAS_ERROR_TBMV(pos, order, Uplo, TransA, Diag, N, K, A, lda, X,      \
                         incX)                                                 \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_UPLO(pos, 2, Uplo);                                                    \
  CHECK_TRANSPOSE(pos, 3, TransA);                                             \
...
/* cblas_xtbsv() */
#define CBLAS_ERROR_TBSV(pos, order, Uplo, TransA, Diag, N, K, A, lda, X,      \
                         incX)                                                 \
...
/* cblas_xsymv() */
#define CBLAS_ERROR_SD_SYMV(pos, order, Uplo, N, alpha, A, lda, X, incX, beta, \
                            Y, incY)                                           \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_UPLO(pos, 2, Uplo);                                                    \
  CHECK_DIM(pos, 3, N);                                                        \
...
/* cblas_xsbmv() */
#define CBLAS_ERROR_SD_SBMV(pos, order, Uplo, N, K, alpha, A, lda, X, incX,    \
                            beta, Y, incY)                                     \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_UPLO(pos, 2, Uplo);                                                    \
  CHECK_DIM(pos, 3, N);                                                        \
...
/* cblas_xspmv() */
#define CBLAS_ERROR_SD_SPMV(pos, order, Uplo, N, alpha, Ap, X, incX, beta, Y,  \
                            incY)                                              \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_UPLO(pos, 2, Uplo);                                                    \
  CHECK_DIM(pos, 3, N);                                                        \
...
/* cblas_xsyr2() */
#define CBLAS_ERROR_SD_SYR2(pos, order, Uplo, N, alpha, X, incX, Y, incY, A,   \
                            lda)                                               \
  CHECK_ORDER(pos, 1, order);                                                  \
  CHECK_UPLO(pos, 2, Uplo);                                                    \
  CHECK_DIM(pos, 3, N);                                                        \
...
/* cblas_xhemv() */
#define CBLAS_ERROR_CZ_HEMV(pos, order, Uplo, N, alpha, A, lda, X, incX, beta, \
                            Y, incY)                                           \
  CBLAS_ERROR_SD_SYMV(pos, order, Uplo, N, alpha, A, lda, X, incX, beta, Y,    \
                      incY)

/* cblas_xhbmv() */
#define CBLAS_ERROR_CZ_HBMV(pos, order, Uplo, N, K, alpha, A, lda, X, incX,    \
                            beta, Y, incY)                                     \
  CBLAS_ERROR_SD_SBMV(pos, order, Uplo, N, K, alpha, A, lda, X, incX, beta, Y, \
                      incY)

/* cblas_xhpmv() */
#define CBLAS_ERROR_CZ_HPMV(pos, order, Uplo, N, alpha, Ap, X, incX, beta, Y,  \
                            incY)                                              \
...
/* cblas_xher2() */
#define CBLAS_ERROR_CZ_HER2(pos, order, Uplo, N, alpha, X, incX, Y, incY, A,   \
                            lda)                                               \
