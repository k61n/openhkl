...

{
  ...
  CHECK_ARGS13(HER2K, Order, Uplo, Trans, N, K, alpha, A, lda, B, ldb, beta, C,
               ldc);
    ...
    if (uplo == CblasUpper && trans == CblasNoTrans) {
      ...
      for (i = 0; i < N; i++) {
        ...
        /* Cii += alpha Aik conj(Bik) + conj(alpha) Bik conj(Aik) */
        {
          ...
          for (k = 0; k < K; k++) {
            ...
            /* temp1 = alpha * Aik */
            const BASE temp1_real =
                alpha_real * Aik_real - alpha_imag * Aik_imag;
            const BASE temp1_imag =
                alpha_real * Aik_imag + alpha_imag * Aik_real;
        ...
        for (j = i + 1; j < N; j++) {
          ...
          for (k = 0; k < K; k++) {
            ...
            /* temp2 = alpha * Ajk */
            const BASE temp2_real =
                alpha_real * Ajk_real - alpha_imag * Ajk_imag;
            const BASE temp2_imag =
                alpha_real * Ajk_imag + alpha_imag * Ajk_real;
      ...
      for (k = 0; k < K; k++) {
        for (i = 0; i < N; i++) {
          ...
          {
            REAL(C, i * lda + i) +=
                2 * (temp1_real * Bki_real - temp1_imag * Bki_imag);
            IMAG(C, i * lda + i) = 0.0;
          }
