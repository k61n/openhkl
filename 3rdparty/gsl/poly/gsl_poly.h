...
INLINE_DECL double gsl_poly_eval(const double c[], const int len,
                                 const double x);

/* real polynomial, complex x */
INLINE_DECL gsl_complex gsl_poly_complex_eval(const double c[], const int len,
                                              const gsl_complex z);

/* complex polynomial, complex x */
INLINE_DECL gsl_complex gsl_complex_poly_complex_eval(const gsl_complex c[],
                                                      const int len,
                                                      const gsl_complex z);
...
gsl_complex gsl_poly_complex_eval(const double c[], const int len,
  ...
  for (i = len - 1; i > 0; i--) {
    /* The following three lines are equivalent to
       ...
       but faster */
    double tmp =
        c[i - 1] + GSL_REAL(z) * GSL_REAL(ans) - GSL_IMAG(z) * GSL_IMAG(ans);
    GSL_SET_IMAG(&ans,
                 GSL_IMAG(z) * GSL_REAL(ans) + GSL_REAL(z) * GSL_IMAG(ans));
...
INLINE_FUN
gsl_complex gsl_complex_poly_complex_eval(const gsl_complex c[], const int len,
                                          const gsl_complex z) {
  int i;
  ...
  for (i = len - 1; i > 0; i--) {
    ...
    GSL_SET_IMAG(&ans, GSL_IMAG(c[i - 1]) + GSL_IMAG(z) * GSL_REAL(ans) +
                           GSL_REAL(z) * GSL_IMAG(ans));
    GSL_SET_REAL(&ans, tmp);
  }
  return ans;
}
...
int gsl_poly_dd_init(double dd[], const double x[], const double y[],
                     size_t size);

INLINE_DECL double gsl_poly_dd_eval(const double dd[], const double xa[],
                                    const size_t size, const double x);

#ifdef HAVE_INLINE
INLINE_FUN
double gsl_poly_dd_eval(const double dd[], const double xa[], const size_t size,
                        const double x) {
  size_t i;
  double y = dd[size - 1];
  for (i = size - 1; i--;)
...
int gsl_poly_dd_taylor(double c[], double xp, const double dd[],
                       const double x[], size_t size, double w[]);

int gsl_poly_dd_hermite_init(double dd[], double z[], const double xa[],
 ...
 */
int gsl_poly_solve_quadratic(double a, double b, double c, double *x0,
                             double *x1);
 ...
 * Roots are returned ordered.
 */
int gsl_poly_solve_cubic(double a, double b, double c, double *x0, double *x1,
                         double *x2);

int gsl_poly_complex_solve_cubic(double a, double b, double c, gsl_complex *z0,
                                 gsl_complex *z1, gsl_complex *z2);
