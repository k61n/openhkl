...
template <typename Scalar> struct round_retval { typedef Scalar type; };
...
template <typename Scalar> struct log1p_retval { typedef Scalar type; };
...
template <typename T>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename NumTraits<T>::Real
abs(const T &x) {
  EIGEN_USING_STD_MATH(abs);
  return abs(x);
}
