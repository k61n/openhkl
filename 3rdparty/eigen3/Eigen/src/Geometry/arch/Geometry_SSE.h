...
struct quat_conj<Architecture::SSE, Derived, float, Alignment> {
  static inline Quaternion<float> run(const QuaternionBase<Derived> &q) {
    ...
    const __m128 mask = _mm_setr_ps(-0.f, -0.f, -0.f, 0.f);
    pstore(&res.x(),
           _mm_xor_ps(mask, q.coeffs().template packet<Alignment>(0)));
    return res;
  }
};
...
struct quat_conj<Architecture::SSE, Derived, double, Alignment> {
  static inline Quaternion<double> run(const QuaternionBase<Derived> &q) {
    ...
    pstore(&res.x(),
           _mm_xor_pd(mask0, q.coeffs().template packet<Alignment>(0)));
    pstore(&res.z(),
           _mm_xor_pd(mask2, q.coeffs().template packet<Alignment>(2)));
    return res;
  }
};
