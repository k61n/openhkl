#define BOOST_TEST_MODULE "Test 2D Object oriented Bounding Box"
...
BOOST_AUTO_TEST_CASE(Test_OBB_2D)
{
    // Test: the construction of an OBB
    Eigen::Vector2d center(3,4);
    Eigen::Vector2d extent(sqrt(2),sqrt(2));
    Eigen::Matrix2d axis;
    axis << 1,0,
            0,1;
    OBB<double,2> obb1(center,extent,axis);
    Eigen::Vector2d lower(obb1.getLower());
    Eigen::Vector2d upper(obb1.getUpper());
    ...
    obb3.translate(Eigen::Vector2d(-1,2));
    lower = obb3.getLower();
    upper = obb3.getUpper();
    ...
    axis << 1,0,
            0,1;
    OBB<double,2> obb4(Eigen::Vector2d(1,1),Eigen::Vector2d(2,2),axis);

    double t1,t2;
    BOOST_CHECK(obb4.rayIntersect(Eigen::Vector2d(-5,0),Eigen::Vector2d(2,0.5),t1,t2));
    BOOST_CHECK_CLOSE(t1, 2.0, 1.0e-10);
    BOOST_CHECK_CLOSE(t2, 4.0, 1.0e-10);
    BOOST_CHECK(obb4.rayIntersect(Eigen::Vector2d(-3,3),Eigen::Vector2d(4,-2),t1,t2));
    BOOST_CHECK_CLOSE(t1, 0.5, 1.0e-10);
    BOOST_CHECK_CLOSE(t2, 1.5, 1.0e-10);
    BOOST_CHECK(!obb4.rayIntersect(Eigen::Vector2d(-6,8),Eigen::Vector2d(2,3),t1,t2));

    Eigen::Vector2d s(sqrt(2),sqrt(2));
    obb4.scale(s);
    ...
    BOOST_CHECK(obb4.rayIntersect(Eigen::Vector2d(-4,-2),Eigen::Vector2d(1.5,0.5),t1,t2));
    BOOST_CHECK_CLOSE(t1, 2.0, 1.0e-10);
    BOOST_CHECK_CLOSE(t2, 6.0, 1.0e-10);
    BOOST_CHECK(obb4.rayIntersect(Eigen::Vector2d(5,2.5),Eigen::Vector2d(-0.5,0),t1,t2));
    ...
    BOOST_CHECK(!obb4.rayIntersect(Eigen::Vector2d(-6,8),Eigen::Vector2d(2,5),t1,t2));
}
