...
int main() {
  ...
  for (auto i = -N; i < N; ++i) {
    for (auto j = -N; j < N; ++j) {
      for (auto k = -N; k < N; ++k) {
        Eigen::Vector3d u(center(0) + i * dx, center(1) + j * dx,
                          center(2) + k * dx);

        NSX_CHECK_EQUAL(f.isInside(u),
                        (u - center).transpose() * f.metric() * (u - center) <
                            1.0);
  ...
  nsx::AABB aabb(Eigen::Vector3d(-20, -20, -20),
                 Eigen::Vector3d(-16, -16, -16));
  ...
  nsx::Ellipsoid el(Eigen::Vector3d(3, -3, 0),
                    Eigen::Vector3d(1.0, 20.0, 100.0), U);

  // check collision with line segments
  NSX_CHECK_EQUAL(
      el.collideSegment(Eigen::Vector3d(3, -3, 0), Eigen::Vector3d(3, -100, 0)),
      true);
  NSX_CHECK_EQUAL(
      el.collideSegment(Eigen::Vector3d(5, -5, 0), Eigen::Vector3d(1, -1, 0)),
      true);
