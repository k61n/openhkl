...
int main() {
  ...
  NSX_CHECK_EQUAL(imgr->property<std::complex<double>>("Hf[176]", "b_coherent"),
  ...
  NSX_CHECK_CLOSE(imgr->property<double>("H[1]", "molar_mass"),
                  1.00782504 * um->get("g_per_mole"), tolerance);
  NSX_CHECK_EQUAL(imgr->property<int>("H[1]", "n_protons"), 1);
  NSX_CHECK_ASSERT(imgr->property<bool>("H[1]", "stable"));
  NSX_CHECK_ASSERT(imgr->property<std::string>("H[1]", "symbol").compare("H") ==
                   0);
