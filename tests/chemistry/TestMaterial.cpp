...
{
    BOOST_CHECK_THROW(nsx::Material junk("X"),std::runtime_error);
    ...
    BOOST_CHECK_CLOSE(atomicDensity.at("H[1]") , nsx::avogadro*2.0*0.999850*water.massDensity()/waterMolarMass, tolerance);
    BOOST_CHECK_CLOSE(atomicDensity.at("H[2]") , nsx::avogadro*2.0*0.000150*water.massDensity()/waterMolarMass, tolerance);
    ...
    BOOST_CHECK_CLOSE(water.muIncoherent(), nsx::barn*(dO16*0.0+dO17*0.004+dO18*0.0+dH1*80.270+dH2*2.05), tolerance);
    BOOST_CHECK_CLOSE(water.muAbsorption(4.8*nsx::ang), nsx::barn*(dO16*0.0001+
