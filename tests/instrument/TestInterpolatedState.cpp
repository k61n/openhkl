...
void run_test(const char *filename, const char *instrument) {
  ...
  for (auto coord : coords) {
    ...
    NSX_CHECK_SMALL((dq1 - Jq * Eigen::Vector3d(dt, 0, 0)).norm() / dq1.norm(),
                    1e-3);
    NSX_CHECK_SMALL((dq2 - Jq * Eigen::Vector3d(0, dt, 0)).norm() / dq2.norm(),
                    1e-3);
    NSX_CHECK_SMALL((dq3 - Jq * Eigen::Vector3d(0, 0, dt)).norm() / dq3.norm(),
                    1e-3);
