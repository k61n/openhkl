...
int main() {
  ...
  NSX_CHECK_CLOSE(cell4.angle({1, 0, 0}, {0, 0, 1}), 82.0 * nsx::deg,
                  tolerance);
  ...
  NSX_CHECK_ASSERT(space_group.isEquivalent(nsx::MillerIndex(2, 0, 0),
                                            nsx::MillerIndex(0, 2, 0)));
  NSX_CHECK_ASSERT(space_group.isEquivalent(nsx::MillerIndex(2, 3, 2),
                                            nsx::MillerIndex(3, 2, -2)));
  NSX_CHECK_ASSERT(!space_group.isEquivalent(nsx::MillerIndex(2, 3, 2),
                                             nsx::MillerIndex(3, 2, -3)));
  ...
  for (auto n = 0; n < nmax; ++n) {
    ...
    new_cell.setMetric(ch.g00 + d[0], ch.g01 + d[5], ch.g02 + d[4],
                       ch.g11 + d[1], ch.g12 + d[3], ch.g22 + d[2]);
  ...
  cell =
      nsx::UnitCell(5.557, 5.77, 16.138, 96.314 * deg, 90.0 * deg, 90.0 * deg);
