...
\begin{equation}
  \delta\q
  = \delta k_f - \delta k_i = \bu(k_i \cdot \delta k_i) / |k_i| - \delta k_i
  =: A \delta k_i,
\end{equation}
where $A$ is the matrix $A = |k_i|^{-1} \bu k_i^t - \v1 = |k_i|^{-2} k_f k_i^t - \v1$. Note that $A k_i = \q$ and therefore
$\q + \delta \q = A(k_i + \delta k_i)$.

Therefore, the observed intensity at detector position $(x,y)$ should be proportional to
\begin{align}
  &= \int f(\q - \q_{hkl} + \delta \q) p(\delta k_i) d(\delta k_i) \\
  &= \int f(\q - \q_{hkl} + RA \delta k_i) p(\delta k_i) d(\delta k_i)
\end{align}

where $R$ is the rotation matrix taking lab coordinates to sample-fixed coordinates.
\textbf{Assuming that $A$ is invertible (why? $\det A = -\frac{1}{2}\q^2$)}, we have $\delta k_i = A^{-1} \delta \q$ and $d(\delta k_i) = |\det A|^{-1} d(\delta \q)$. Let $\Sigma_M$ denote the variance-covariance matrix of the profile shape $f$ and let $\Sigma_D$ denote the variance-covariance matrix of the beam divergence $\delta k_i$. Then from the above formula we see that the \emph{observed} profile shape will have (in sample-fixed q-space) a variance-covariance matrix given by
\begin{equation} \Sigma_M + R A \Sigma_D A^t R^t, \end{equation}
where $R$ is the rotation matrix from lab space to sample space and $A = |k_i|^{-2} k_f k_i^t - \v1$.
...
To better understand $\be_3$, consider the following: we want to find the axis $\ba$ such that $\q$ passes through the Ewald sphere as fast as possible. Hence, we want to maximize $(\ba \times \q) \cdot k_f$ subject to the constraint $\ba \cdot \ba = 1$.
Now $(\ba \times \q) \cdots k_f) = \ba \cdot (\ba \times k_f) = \ba \cdot (k_f \times k_i)$, so by the method of Langrange multipliers we must solve $k_f \times k_i = \lambda \ba$, which tells us immediately that the axis is in the direction of $\be_1$.
...
where $x = \Sigma^{-1} \begin{bmatrix} 1 \\ 1 \end{bmatrix}$
and $\Sigma$ is the variance-covariance matrix of $\Ifit$ and $\Isum$.
