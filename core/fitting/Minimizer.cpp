...
static void error_handler(const char *reason, const char *file, int line,
                          int gsl_errno) {
  std::string msg = file;
...
void callback_helper(const size_t /*iter*/, void * /*data*/,
...
struct MinimizerGSL {
  ...
  MinimizerGSL()
      : workspace(nullptr), jacobian(nullptr), covariance(nullptr), x(nullptr),
        wt(nullptr){};
};
...
Minimizer::Minimizer()
    : _gsl(new MinimizerGSL), _numValues(0), _xtol(1e-7), _gtol(1e-7),
      _ftol(1e-7), _f(nullptr), _df(nullptr)
...
void Minimizer::initialize(FitParameters &params, int values) {
  ...
  _gsl->workspace = gsl_multifit_nlinear_alloc(
      gsl_multifit_nlinear_trust, &_gsl->fdfParams, _numValues, nfree);
}
...
bool Minimizer::fit(int max_iter) {
  ...
  _gsl->fdf.params =
      this; // this is the data ptr which is passed to gsl_f_wrapper
  ...
  _gsl->status = gsl_multifit_nlinear_driver(max_iter, _xtol, _gtol, _ftol,
                                             &callback_helper, this,
                                             &_gsl->info, _gsl->workspace);
...
const char *Minimizer::getStatusStr() { return gsl_strerror(_gsl->status); }

int Minimizer::gsl_f_wrapper(const gsl_vector *input, void *data,
                             gsl_vector *output) {
  // call the function
...
int Minimizer::gsl_df_wrapper(const gsl_vector *input, void *data,
                              gsl_matrix *output) {
  // call the function
  Minimizer *self = reinterpret_cast<Minimizer *>(data);
...
void Minimizer::setxTol(double xtol) { _xtol = xtol; }

void Minimizer::setgTol(double gtol) { _gtol = gtol; }

void Minimizer::setfTol(double ftol) { _ftol = ftol; }
