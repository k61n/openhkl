...
RefinementBatch::RefinementBatch(InstrumentStateList &states,
                                 const UnitCell &uc, const PeakList &peaks)
    : _fmin(std::numeric_limits<double>().max()),
      _fmax(std::numeric_limits<double>().lowest()), _cell(new UnitCell(uc)),
      _peaks(peaks) {
  for (auto peak : peaks) {
...
void RefinementBatch::refineDetectorOffset() {
  for (int axis = 0; axis < 3; ++axis) {
    ...
    for (size_t i = 0; i < _states.size(); ++i) {
      int id = _params.addParameter(
          &(_states[i].get().detectorPositionOffset(axis)));
      ids.push_back(id);
    }
...
void RefinementBatch::refineSampleOrientation() {
  ...
  for (int axis = 0; axis < 3; ++axis) {
    ...
    for (size_t i = 0; i < _states.size(); ++i) {
      int id = _params.addParameter(
          &(_states[i].get().sampleOrientationOffset.coeffs()[axis]));
      ids.push_back(id);
    }
    // record the constraints
    _constraints.push_back(ids);
...
const PeakList &RefinementBatch::peaks() const { return _peaks; }

sptrUnitCell RefinementBatch::cell() const { return _cell; }
