...
Refiner::Refiner(InstrumentStateList &states, sptrUnitCell cell,
                 const PeakList &peaks, int nbatches)
    : _batches(), _cell(cell) {
...
int Refiner::updatePredictions(PeakList &peaks) const {
  ...
  filtered_peaks =
      peak_filter.indexed(filtered_peaks, *_cell, _cell->indexingTolerance());
  ...
  for (auto &&peak : filtered_peaks) {
    ...
    ReciprocalVector q_pred(hkl.rowVector().cast<double>() *
                            batch_cell->reciprocalBasis());
    ...
    try {
      peak->setShape(Ellipsoid({events[0]._px, events[0]._py, events[0]._frame},
                               peak->shape().metric()));
      ++updated;
    } catch (...) {
      peak->setSelected(false);
