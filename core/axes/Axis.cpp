...
Axis::Axis()
    : _name("axis"), _axis(Eigen::Vector3d(0.0, 0.0, 1.0)), _physical(true),
      _id(0) {}

Axis::Axis(const std::string &name)
    : _name(name), _axis(Eigen::Vector3d(0.0, 0.0, 1.0)), _physical(true),
      _id(0) {}
...
Axis::Axis(const Axis &other)
    : _name(other._name), _axis(other._axis), _physical(other._physical),
      _id(other._id) {}
...
void Axis::setName(const std::string &name) { _name = name; }

const std::string &Axis::name() const { return _name; }
...
const Eigen::Vector3d &Axis::axis() const { return _axis; }

void Axis::setId(unsigned int id) { _id = id; }

unsigned int Axis::id() const { return _id; }
...
void Axis::setPhysical(bool physical) { _physical = physical; }

bool Axis::physical() const { return _physical; }
