...
Axis *RotAxis::create(const YAML::Node &node) { return new RotAxis(node); }

RotAxis::RotAxis() : Axis("rotation"), _dir(Direction::CCW) {}

RotAxis::RotAxis(const std::string &label)
    : Axis(label), _dir(Direction::CCW) {}

RotAxis::RotAxis(const std::string &label, const Eigen::Vector3d &axis,
                 Direction direction)
...
RotAxis *RotAxis::clone() const { return new RotAxis(*this); }

void RotAxis::setRotationDirection(Direction dir) { _dir = dir; }

RotAxis::Direction RotAxis::rotationDirection() const { return _dir; }
...
RotAxis::affineMatrix(double angle) const {
