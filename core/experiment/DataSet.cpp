...
void DataSet::saveHDF5(const std::string& filename) // const
{
    ...
    cd_values[6] =
        BLOSC_BLOSCLZ; // Seem to be the best compromise speed/compression for diffraction data
    ...
    for (offset[0] = 0; offset[0] < _nFrames; offset[0] += count[0]) {
        ...
        Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> current_frame(
            frame(offset[0]));
