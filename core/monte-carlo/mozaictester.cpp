...
//
//! @file      core/monte-carlo/mozaictester.cpp
...
bool intersect(
    double p0, double p1, double p2, double u0, double u1, double u2, double d, double radius,
    double& t1, double& t2)
{
...
Eigen::Vector3d MozaicTester::buildQVector(
    ...
    for (int i = 0; i <= nPtsMonoX; ++i) {
        ...
        for (int j = 0; j <= nPtsMonoY; ++j) {
            ...
            for (int k = 0; k < nPtsAlpha2; ++k) {
                ...
                // sin(omega)
                double a = -n[0] * (u[0] * ca2 + v[0] * sa2) - n[1] * (u[1] * ca2 + v[1] * sa2)
                    - n[2] * (u[2] * ca2 + v[2] * sa2);
                double b = -n.dot(w);
                ...
                if (lambda < lmin)
                    ...
                    if (alpha1 <= murad && alpha1 >= 0.0) {
                        updateDetector(
                            alpha1, znorm, ca2, sa2, lmin, u, v, w, n, px, py, pz, detector);
                        continue;
                    }
                    ...
                    if (alpha1 <= murad && alpha1 >= 0.0) {
                        updateDetector(
                            alpha1, znorm, ca2, sa2, lmax, u, v, w, n, px, py, pz, detector);
                        continue;
                    }
...
Eigen::MatrixXd MozaicTester::run(double h, double k, double l, int n)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        double t1, t2;
        bool test = intersect(
            xMono, 0.0, zMono, xSample - xMono, _dMono - _sRadius, zSample - zMono, _dMono,
            _sRadius, t1, t2);
        if (!test)
            continue;
        ...
        Eigen::Vector3d from(
            xMono + t * (xSample - xMono), t * (_dMono - _sRadius), zMono + t * (zSample - zMono));
...
Eigen::MatrixXd MozaicTester::runContour(double h, double k, double l)
{
    ...
    for (int c = 0; c < nCones1; c++) {
        ...
        for (int c2 = 0; c2 <= nCones2; c2++) {
            double step2 =
                cos(_mu * M_PI / 180.0) + (double)c2 / nCones2 * (1 - cos(_mu * M_PI / 180.0));
            Eigen::Vector3d v = rotTheta
                * Eigen::Vector3d(znorm * step2, znorm * sqrt(1 - step2 * step2) * cos(step),
