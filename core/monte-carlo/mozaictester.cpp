    ...
    for (int i = 0; i <= nPtsMonoX; ++i) {
        ...
        for (int j = 0; j <= nPtsMonoY; ++j) {
            ...
            for (int k = 0; k < nPtsAlpha2; ++k) {
                ...
                // We have sin(theta) = lambda*q/2 = cos(pi/2-theta) = cos(u,Q)//||Q|| with
                // u=MP/||MP|| hence uzcos(alpha1) + (uxcos(alpha2)+uycos(alphap2))*sin(alpha1) =
                // lambda*q/2 which is an equation of the form sin(phi)cos(alpha1) +
                // cos(phi)sin(alpha1) = sin(omega)
...
Eigen::MatrixXd MozaicTester::runCenter(double h, double k, double l, int n)
{
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // Compute the 'real' wavelength that corresponds to this Q vector on which we applied the
        // mosaicity
