...
Eigen::Vector3d MozaicTester::buildQVector(double alpha1, double ca2,
                                           double sa2, double znorm,
...
void MozaicTester::updateDetector(
    double alpha1, double znorm, double calpha2, double salpha2, double lambda,
    const Eigen::Vector3d &u, const Eigen::Vector3d &v,
    const Eigen::Vector3d &w, const Eigen::Vector3d &n, double px, double py,
    double pz, Eigen::MatrixXd &detector) {

  Eigen::Vector3d qvect =
      buildQVector(alpha1, calpha2, salpha2, znorm, u, v, w);
...
Eigen::MatrixXd MozaicTester::run1(double h, double k, double l, double px,
                                   double py, double pz) {
  ...
  // Create a w vector along x axis and rotate in xy plane with the Bragg angle
  // defined above
  Eigen::Vector3d w;
  w << 1, 0, 0;
  w = rottheta * w;
  ...
  for (int i = 0; i <= nPtsMonoX; ++i) {
    ...
    for (int j = 0; j <= nPtsMonoY; ++j) {
      ...
      Eigen::Vector3d mp(px + _wMono / 2.0 - stepi, py + _dMono,
                         pz + _hMono / 2.0 - stepj);
      ...
      for (int k = 0; k < nPtsAlpha2; ++k) {
        ...
        // We have sin(theta) = lambda*q/2 = cos(pi/2-theta) = cos(u,Q)//||Q||
        // with u=MP/||MP|| hence uzcos(alpha1) +
        // (uxcos(alpha2)+uycos(alphap2))*sin(alpha1) = lambda*q/2 which is an
        // equation of the form sin(phi)cos(alpha1) + cos(phi)sin(alpha1) =
        // sin(omega)
        double a = -n[0] * (u[0] * ca2 + v[0] * sa2) -
                   n[1] * (u[1] * ca2 + v[1] * sa2) -
                   n[2] * (u[2] * ca2 + v[2] * sa2);
        double b = -n.dot(w);
        ...
        if (lambda < lmin)
          ...
          if (alpha1 <= murad && alpha1 >= 0.0) {
            updateDetector(alpha1, znorm, ca2, sa2, lmin, u, v, w, n, px, py,
                           pz, detector);
            continue;
          }
          ...
          alpha1 = omega - phi;
          if (alpha1 <= murad && alpha1 >= 0.0) {
            updateDetector(alpha1, znorm, ca2, sa2, lmax, u, v, w, n, px, py,
                           pz, detector);
            continue;
          }
...
Eigen::MatrixXd MozaicTester::runCenter(double h, double k, double l, int n) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    // Build a Q vector along X axis on which a mosaicity is applied to get it
    // out its axis
    Eigen::Vector3d qvect(znorm * ca, znorm * sa * cos(beta),
                          znorm * sa * sin(beta));
    ...
    // Compute the 'real' wavelength that corresponds to this Q vector on which
    // we applied the mosaicity
...
Eigen::MatrixXd MozaicTester::run(double h, double k, double l, int n) {
  ...
  for (int i = 0; i < 1000; ++i) {
    ...
    lambs[i] = 1.0 / sqrt(2.0 * M_PI) / _dl *
               exp(-0.5 * (x - _l) * (x - _l) / _dl / _dl);
  }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    double t1, t2;
    bool test = intersect(xMono, 0.0, zMono, xSample - xMono, _dMono - _sRadius,
                          zSample - zMono, _dMono, _sRadius, t1, t2);
    if (!test)
      continue;

    double t = t1 + xor128() * (t2 - t1);

    Eigen::Vector3d from(xMono + t * (xSample - xMono), t * (_dMono - _sRadius),
                         zMono + t * (zSample - zMono));

    double alpha = murad + xor128() * (1.0 - murad);
    double beta = -M_PI + xor128() * 2.0 * M_PI;
    double sa = sqrt(1 - alpha * alpha);
    Eigen::Vector3d qvect(znorm * alpha, znorm * sa * cos(beta),
                          znorm * sa * sin(beta));
    qvect = rotTheta * qvect;
...
Eigen::MatrixXd MozaicTester::runContour(double h, double k, double l) {
  ...
  for (int c = 0; c < nCones1; c++) {
    ...
    for (int c2 = 0; c2 <= nCones2; c2++) {
      double step2 = cos(_mu * M_PI / 180.0) +
                     (double)c2 / nCones2 * (1 - cos(_mu * M_PI / 180.0));
      Eigen::Vector3d v =
          rotTheta *
          Eigen::Vector3d(znorm * step2,
                          znorm * sqrt(1 - step2 * step2) * cos(step),
                          znorm * sqrt(1 - step2 * step2) * sin(step));
      qvects.push_back(v);
    }
  }
  ...
  for (int th = 0; th <= nstacks; ++th) {
    ...
    for (int ph = 0; ph < nslices; ++ph) {
      ...
      double sp = sin(phi);
      double cp = cos(phi);
      Eigen::Vector3d to(_sRadius * stheta * cp, _sRadius * stheta * sp,
                         _sRadius * ctheta);
      tos.push_back(to);
    }
  }
  ...
  for (int i = 0; i <= Npoints; ++i) {
    ...
    for (int j = 0; j <= Npoints; ++j) {
      ...
      Eigen::Vector3d from(-_wMono / 2.0 + stepi, -_dMono,
                           -_hMono / 2.0 + stepj);
      Eigen::Vector3d ki;
      for (auto &to : tos) {
        ki = (to - from);
        ...
        for (auto &qvect : qvects) {
          ...
          if (ll >= _l - 0.5 * _dl && ll <= _l + 0.5 * _dl) {
            double kfx = rotTheta2(0, 0) * (ki[0] / ll + qvect[0]) +
                         rotTheta2(0, 1) * (ki[1] / ll + qvect[1]);
            double kfy = rotTheta2(1, 0) * (ki[0] / ll + qvect[0]) +
                         rotTheta2(1, 1) * (ki[1] / ll + qvect[1]);
