...
MCAbsorption::MCAbsorption(const ConvexHull &convex_hull, double source_width,
                           double source_height, double source_y_pos)
    : _convex_hull(convex_hull), _mu_scattering(1.0), _mu_absorption(1.0),
      _source_width(source_width), _source_height(source_height),
...
double MCAbsorption::run(unsigned int nIterations, const Eigen::Vector3d &outV,
                         const Eigen::Matrix3d &sampleOrientation) const {
  std::function<double(void)> random =
      std::bind(std::uniform_real_distribution<double>(-0.5, 0.5),
