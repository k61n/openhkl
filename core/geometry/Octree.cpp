...
Octree::Octree(Octree &&other)
    : _MAX_DEPTH(other._MAX_DEPTH), _MAX_STORAGE(other._MAX_STORAGE),
      _MULTIPLICITY(other._MULTIPLICITY), _POWERS(std::move(other._POWERS)),
      _children(std::move(other._children)), _data(std::move(other._data)),
      _depth(other._depth), _parent(other._parent), _idx(other._idx) {}

void Octree::nullifyChildren() { _children.clear(); }

Octree::Octree() : AABB(), _depth(0), _parent(nullptr) { nullifyChildren(); }
...
Octree::Octree(const Octree *parent, unsigned int sector)
    : AABB(), _MAX_DEPTH(parent->_MAX_DEPTH),
      _MAX_STORAGE(parent->_MAX_STORAGE), _depth(parent->_depth + 1),
      _parent(parent), _idx(sector) {
  nullifyChildren();
...
bool Octree::hasChildren() const { return (!_children.empty()); }

bool Octree::hasData() const { return (_data.size() != 0); }
...
std::set<const Ellipsoid *>
Octree::getCollisions(const Ellipsoid &given) const {
  ...
  recursiveCollisions =
      [&given, &recursiveCollisions](const Octree *tree,
                                     CollisionSet &collisions) -> void {
...
void Octree::printSelf(std::ostream &os) const {
  os << "*** Node ***  " << this->_lowerBound << "," << this->_upperBound
     << std::endl;
  if (!hasChildren()) {
...
Octree::iterator Octree::begin() const { return iterator(*this); }

Octree::iterator Octree::end() const { return iterator(); }
...
const Octree &OctreeIterator::operator*() const { return *_node; }
