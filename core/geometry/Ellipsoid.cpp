...
Ellipsoid::Ellipsoid(const Eigen::Vector3d &center,
                     const Eigen::Matrix3d &metric)
    : _center(center), _metric(metric), _inverseMetric(metric.inverse()),
      _aabb() {
  updateAABB();
}
...
const Eigen::Vector3d &Ellipsoid::center() const { return _center; }

const Eigen::Matrix3d &Ellipsoid::metric() const { return _metric; }
...
const AABB &Ellipsoid::aabb() const { return _aabb; }
...
bool Ellipsoid::collideSegment(const Eigen::Vector3d &a,
...
bool Ellipsoid::collideFace(const Eigen::Vector3d &o, const Eigen::Vector3d &a,
...
Eigen::Vector3d Ellipsoid::intersectionCenter(const Eigen::Vector3d &n,
...
Ellipsoid Ellipsoid::toDetectorSpace(sptrDataSet data) const {
  ...
  if (events.size() != 1) {
    throw std::runtime_error(
        "could not transform ellipse from q space to detector space");
  }
