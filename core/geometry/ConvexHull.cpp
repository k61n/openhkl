...
ConvexHull::ConvexHull(const ConvexHull &other) { *this = other; }
...
ConvexHull &ConvexHull::operator=(const ConvexHull &other) {
  if (this != &other) {
    ...
    for (auto p : _vertices) {
      ...
      new_vertex->_duplicate = other_vertex->_duplicate
                                   ? _edges[other_vertex->_duplicate->_id]
                                   : nullptr;
    }
    ...
    for (auto p : _edges) {
      ...
      new_edge->_newFace =
          other_edge->_newFace ? _faces[other_edge->_newFace->_id] : nullptr;
...
ConvexHull::ConvexHull()
    : _initialized(false), _vertices(), _edges(), _faces(), _planes(),
      _innerR2(0.0), _outerR2(0.0), _vertex_id(0), _edge_id(0), _face_id(0) {}

ConvexHull::~ConvexHull() { reset(); }
...
bool ConvexHull::findInitialVertices(int &ri, int &rj, int &rk) const {
  if (_vertices.size() < 3) {
    throw std::runtime_error(
        "Can not set the initial polytope with less than 3 vertices.");
  }
...
const std::map<int, Edge *> &ConvexHull::edges() const { return _edges; }

const std::map<int, Face *> &ConvexHull::faces() const { return _faces; }
...
size_t ConvexHull::nVertices() const { return _vertices.size(); }

size_t ConvexHull::nEdges() const { return _edges.size(); }

size_t ConvexHull::nFaces() const { return _faces.size(); }
...
std::vector<Triangle>
ConvexHull::createFaceCache(const Eigen::Matrix3d &rotation) const {
  if (_vertices.size() < 4) {
    throw std::runtime_error(
        "Hull is flat or undefined, can not construct faces information");
  }
