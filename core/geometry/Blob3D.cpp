...
Blob3D::Blob3D()
    : _m0(0), _m1(Eigen::Vector3d::Zero()), _m2(Eigen::Matrix3d::Zero()),
      _npoints(0), _minValue(std::numeric_limits<double>::max()),
      _maxValue(std::numeric_limits<double>::min()) {}

Blob3D::Blob3D(const Blob3D &b)
    : _m0(b._m0), _m1(b._m1), _m2(b._m2), _npoints(b._npoints),
...
double Blob3D::getMass() const { return _m0; }
int Blob3D::getComponents() const { return _npoints; }

double Blob3D::getMinimumMass() const { return _minValue; }

double Blob3D::getMaximumMass() const { return _maxValue; }
...
void Blob3D::toEllipsoid(double scale, Eigen::Vector3d &c,
