...
bool AABB::collide(const AABB &aabb) const {
  for (unsigned int i = 0; i < 3; ++i) {
    if (_upperBound(i) < aabb._lowerBound(i) ||
        _lowerBound(i) > aabb._upperBound(i))
      return false;
  }
  return true;
}
...
void AABB::setLower(const Eigen::Vector3d &lower) { _lowerBound = lower; }

void AABB::setUpper(const Eigen::Vector3d &upper) { _upperBound = upper; }

const Eigen::Vector3d &AABB::lower() const { return _lowerBound; }

const Eigen::Vector3d &AABB::upper() const { return _upperBound; }

Eigen::Vector3d AABB::center() const {
  return (_lowerBound + _upperBound) * 0.5;
}

Eigen::Vector3d AABB::extents() const { return _upperBound - _lowerBound; }
...
bool AABB::contains(const AABB &other) const {
  for (unsigned int i = 0; i < 3; ++i) {
    if (_lowerBound(i) >= other._lowerBound(i) ||
        _upperBound(i) <= other._upperBound(i))
      return false;
  }
  return true;
}
