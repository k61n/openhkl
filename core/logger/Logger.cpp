...
std::string currentTime() { return current_time(); }
...
auto debug_log() -> Logger {
  ...
  wrapper->addWrapper(
      new LogFileStreamWrapper("nsx_debug.txt", initialize, finalize));
...
auto info_log() -> Logger {
  ...
  AggregateStreamWrapper *wrapper = new AggregateStreamWrapper();
  wrapper->addWrapper(
      new LogFileStreamWrapper("nsx_info.txt", initialize, finalize));
  return Logger(wrapper);
}
...
auto error_log() -> Logger {
  ...
  AggregateStreamWrapper *wrapper = new AggregateStreamWrapper();
  wrapper->addWrapper(
      new LogFileStreamWrapper("nsx_error.txt", initialize, finalize));
  return Logger(wrapper);
}
...
Logger debug() { return g_debug(); }

void setDebug(std::function<Logger()> debug) { g_debug = debug; }

Logger info() { return g_info(); }

void setInfo(std::function<Logger()> info) { g_info = info; }

Logger error() { return g_error(); }

void setError(std::function<Logger()> error) { g_error = error; }
