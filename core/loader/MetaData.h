 ...
 *
 *  MetaData class allow to store metadata associated with a data file  into a map indexed by the
 * string of the entry. Any parameter type can be stored, using the corresponding template argument.
...
public:
    ...
    //! Get the value associated with the key. User must know the return type, otherwise use method
    //! that return boost_any
