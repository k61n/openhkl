...
//
//! @file      core/loader/ExperimentMetaReader.cpp
//! @brief     Implements class ExperimentMetaReader
//!
//! @homepage  ###HOMEPAGE###
...
#include "core/loader/ExperimentMetaReader.h"
...
void ExperimentMetaReader::open()
{
    if (_isOpened)
        return;
    ...
    try {
        _file = std::unique_ptr<H5::H5File>(
            new H5::H5File(_metadata.key<std::string>("real_path").c_str(), H5F_ACC_RDONLY));
    } catch (...) {
        if (_file)
            _file.reset();
        throw;
...
void ExperimentMetaReader::close()
{
    if (!_isOpened)
        return;
