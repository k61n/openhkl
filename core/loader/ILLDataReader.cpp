...
ILLDataReader::ILLDataReader(const ILLDataReader& other) : IDataReader(other)
{
    ...
    try {
        boost::interprocess::file_mapping filemap(
            other._filename.c_str(), boost::interprocess::read_only);
    ...
    : IDataReader(filename, diffractometer)
{
    ...
    if ((vi.size() != 2) || (vi[0] != int(_nAngles + 3))) {
        throw std::runtime_error(
            "Problem parsing numor: mismatch between number of angles in header and datablock 1.");
    }

    // This corresponds to the lines going from SSSSSSSSS to "            time         monitor Total
    // Cou     angles*1000"
    ...
    if (vd.size() != (_nAngles + 3)) {
        throw std::runtime_error(
            "Problem parsing numor: mismatch between number of angles in header and datablock 2.");
    }
    _metadata.add<double>("time", vd[0]);
    _metadata.add<double>("monitor", vd[1]);

    // This vector will store the id of the axis scanned by MAD. The id of the ith scanned axis is
    // defined with "icdesci" in the IIIIIIIII metadata block
    ...
    // This map relates the ids of the physical axis registered in the instrument definition file
    // with their name
    ...
    // This map will store the values over the frames of each physical axis of the goniometers bound
    // to the instrument (detector + sample + source) 3 cases:
    //	1) a physical axis is not a scanned axis and its name is not defined in the metadata, then
    //the corresponding values will 	   be a constant equal to 0 	2) a physical axis is not a scanned
    //axis and its name is defined in the metadata, then the corresponding values will 	   be a constant
    //equal to the metadata value
    // 	3) a physical axis is one of the scanned axis, then the corresponding values will be fetched
    // from the FFFFFF dta blocks
    ...
    for (auto&& p : instrument_axis) {
        ...
        // The values for this axis will be a constant
            ...
            // If the axis name is defined in the metadata, the constant will be the corresponding
            // value other wise it will be 0
    ...
    // If a detector is set for this instrument, loop over the frames and gather for each physical
    // axis of the detector the corresponding values defined previously. The gathered values being
    // further pushed as a new detector state
    if (detector) {
        const auto& detector_gonio = _diffractometer->detector()->gonio();
        size_t n_detector_gonio_axes = detector_gonio.nAxes();
        ;
    ...
    // of the sample the corresponding values defined previously. The gathered values being further
    // pushed as a new sample state
    for (size_t i = 0; i < _nFrames; ++i) {
