...
#pragma omp critical
    ...
    // Warning if error
      ...
      if (_handler) {
        _handler->log(std::string("Peak finder caused an exception: ") +
                      e.what());
      }
    ...
    if (_handler) {
      _handler->setStatus(
          ("Integrating " + std::to_string(numor_peaks.size()) + " peaks...")
              .c_str());
      _handler->setProgress(0);
    }
...
void PeakFinder::setMaxFrames(int maxFrames) { _maxFrames = maxFrames; }

int PeakFinder::maxFrames() const { return _maxFrames; }

void PeakFinder::setMinSize(int size) { _minSize = size; }

int PeakFinder::minSize() const { return _minSize; }
...
void PeakFinder::setThreshold(double value) { _threshold = value; }

void PeakFinder::eliminateBlobs(std::map<int, Blob3D> &blobs) const {
  // update progress handler
  if (_handler) {
    _handler->setStatus(
        "Eliminating blobs which are too small or too large...");
    _handler->setProgress(0);
  }
  ...
  if (_handler) {
    _handler->log("After elimination, " + std::to_string(blobs.size()) +
                  " blobs remain");
    _handler->setProgress(100);
  }
}

void PeakFinder::mergeCollidingBlobs(sptrDataSet data,
...
void PeakFinder::findPrimaryBlobs(sptrDataSet data,
                                  std::map<int, Blob3D> &blobs,
...
#pragma omp critical
    ...
    progress = static_cast<double>(nframes) /
               static_cast<double>(data->nFrames()) * 100.0;
...
void PeakFinder::findCollisions(sptrDataSet data, std::map<int, Blob3D> &blobs,
  ...
  Octree oct(Eigen::Vector3d(0.0, 0.0, 0.0),
  ...
  for (auto &&it = collisions.begin(); it != collisions.end(); ++it) {
    ...
    if ((dummy % magic) == 0 && _handler) {
      const double total_dist =
          std::distance(collisions.begin(), collisions.end());
  ...
  if (_handler) {
    _handler->log("Found " + std::to_string(equivalences.size()) +
                  " equivalences");
    _handler->setProgress(100);
  }
...
void PeakFinder::mergeEquivalentBlobs(std::map<int, Blob3D> &blobs,
                                      EquivalenceList &equivalences) const {
  ...
  if (_handler) {
    _handler->log("After merging, " + std::to_string(blobs.size()) +
                  " blobs remain.");
  }
}
