...
ILLDataReader::ILLDataReader(const std::string &filename,
                             Diffractometer *diffractometer)
    : IDataReader(filename, diffractometer) {
  try {
    boost::interprocess::file_mapping filemap(filename.c_str(),
                                              boost::interprocess::read_only);
    _map = boost::interprocess::mapped_region(filemap,
                                              boost::interprocess::read_only);
  } catch (std::exception &e) {
    throw std::runtime_error(std::string("ILLDataReader() caught exception: ") +
                             e.what());
  }
  ...
  size_t predicted_file_size =
      _headerSize + _nFrames * (_skipChar + _dataLength);
  ...
  readIntsFromChar(_mapAddress + _headerSize + skip3Lines,
                   _mapAddress + _headerSize + skip3Lines + 16, vi);
  ...
  for (auto &&p : instrument_axis) {
    ...
    if (it != scannedAxisId.end()) {
      ...
      gonioValues.insert(
          std::pair<unsigned int, std::vector<double>>(p.first, values));
    }
    ...
    // The values for this axis will be a constant
      ...
      double val =
          _metadata.isKey(p.second) ? _metadata.key<double>(p.second) : 0.0;
...
void ILLDataReader::open() {
  ...
  try {
    boost::interprocess::file_mapping filemap(
        _metadata.key<std::string>("filename").c_str(),
        boost::interprocess::read_only);
    boost::interprocess::mapped_region reg(filemap,
                                           boost::interprocess::read_only);
    _map.swap(reg);
    _mapAddress = reinterpret_cast<char *>(_map.get_address());
  } catch (std::exception &e) {
    throw std::runtime_error(
        std::string("ILLAsciiData::open() caught exception: ") + e.what());
  }
  _isOpened = true;
}
...
Eigen::MatrixXi ILLDataReader::data(size_t frame) {
  ...
  std::size_t begin =
      _headerSize + (frame + 1) * _skipChar + frame * _dataLength;
  ...
  MatrixParser parser;
  parser(_diffractometer->detector()->dataOrder(), _mapAddress + begin,
         _dataLength, v);
...
void ILLDataReader::readControlFBlock(std::stringstream &buffer) {
  ...
  std::vector<char> line(
      81, 0); // 81 to make space for null character at string end
  std::string s1, s2;
...
void ILLDataReader::readMetadata(const char *buf) {
  ...
  try {
    ...
    throw std::runtime_error(
        std::string("ILLAsciiData: failed to read header in stream: ") +
        e.what());
  } catch (...) {
    throw std::runtime_error(
        "ILLAsciiData: Fail to read Header in stream (unknown exception)");
  }
  ...
  try {
    ...
    throw std::runtime_error(
        "ILLAsciiMetaReader: Fail to read IBlock in stream");
  }
  ...
  try {
    ...
    throw std::runtime_error(
        "ILLAsciiMetaReader: Fail to read FBlock in stream");
  }
}
