...
TiffDataReader::TiffDataReader(const std::string &filename,
  ...
  if (w != _nCols || h != _nRows) {
    close();
    throw std::range_error("Tiff file " + filename +
                           " not consistent with diffractometer definition");
  }
  _isOpened = false;
  ...
  if (_bits != 16 && _bits != 32) {
    close();
    throw std::runtime_error("Can't read TIFF file " + filename +
                             " : only 16/32bits format supported");
  }
...
Eigen::MatrixXi TiffDataReader::data(std::size_t /*frame*/) {
  ...
  if (_bits == 16) {
    ...
    Eigen::Matrix<uint16, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        data16(_nRows, _nCols);
    // Read line per line
    ...
    Eigen::Matrix<uint32, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>
        data32(_nRows, _nCols);
    // Read line per line
    for (unsigned short int i = 0; i < _nRows; ++i) {
