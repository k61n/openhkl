...
HDF5MetaDataReader::HDF5MetaDataReader(const std::string &filename,
                                       Diffractometer *diffractometer)
    : IDataReader(filename, diffractometer), _dataset(nullptr), _space(nullptr),
      _memspace(nullptr) {
  ...
  for (size_t i = 0; i < n_detector_gonio_axes; ++i) {
    ...
    if (axis.physical()) {
      try {
        ...
        throw std::runtime_error("Coud not read " + axis.name() +
                                 " HDF5 dataset");
      }
    } else {
...
void HDF5MetaDataReader::open() {
  ...
  try {
    _file = std::unique_ptr<H5::H5File>(new H5::H5File(
        _metadata.key<std::string>("filename").c_str(), H5F_ACC_RDONLY));
  } catch (...) {
    if (_file) {
      _file.reset();
    }
  ...
  try {
    _dataset = std::unique_ptr<H5::DataSet>(
        new H5::DataSet(_file->openDataSet("/Data/Counts")));
    // Dataspace of the dataset /counts
    _space =
        std::unique_ptr<H5::DataSpace>(new H5::DataSpace(_dataset->getSpace()));
  } catch (...) {
    throw;
  }
  // Get rank of data
  ...
  count[2] = _nCols;
  _memspace =
      std::unique_ptr<H5::DataSpace>(new H5::DataSpace(3, count, nullptr));
  _isOpened = true;
