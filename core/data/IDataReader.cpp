...
IDataReader::IDataReader(const std::string &filename,
    ...
    : _diffractometer(std::move(diffractometer)), _nFrames(0), _sampleStates(),
      _detectorStates(), _fileSize(0), _isOpened(false) {
...
size_t IDataReader::nFrames() const { return _nFrames; }

size_t IDataReader::nCols() const { return _nCols; }

size_t IDataReader::nRows() const { return _nRows; }

const MetaData &IDataReader::metadata() const { return _metadata; }

MetaData &IDataReader::metadata() { return _metadata; }

const Diffractometer *IDataReader::diffractometer() const {
  return _diffractometer;
}

Diffractometer *IDataReader::diffractometer() { return _diffractometer; }
...
InstrumentState IDataReader::state(size_t frame) const {
  ...
  state.ni.normalize();
  state.wavelength =
      _diffractometer->source().selectedMonochromator().wavelength();
...
bool IDataReader::isOpened() const { return _isOpened; }

std::size_t IDataReader::fileSize() const { return _fileSize; }
