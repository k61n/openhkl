...
DataSet::DataSet(std::shared_ptr<IDataReader> reader)
    : _isOpened(false), _filename(reader->filename()), _nFrames(0), _nrows(0),
      _ncols(0), _data(), _states(), _fileSize(0), _masks(), _background(0.0),
      _reader(reader) {
...
Eigen::MatrixXi DataSet::frame(std::size_t idx) { return _reader->data(idx); }

Eigen::MatrixXd
DataSet::convolvedFrame(std::size_t idx, const std::string &convolver_type,
...
void DataSet::open() { _reader->open(); }

void DataSet::close() { _reader->close(); }

const std::string &DataSet::filename() const { return _filename; }

std::size_t DataSet::nFrames() const { return _nFrames; }

std::size_t DataSet::nCols() const { return _ncols; }

std::size_t DataSet::nRows() const { return _nrows; }
...
std::vector<InstrumentState> &DataSet::instrumentStates() { return _states; }

bool DataSet::isOpened() const { return _isOpened; }

std::size_t DataSet::fileSize() const { return _fileSize; }
...
void DataSet::saveHDF5(const std::string &filename) // const
{
  ...
  if (r <= 0) {
    throw std::runtime_error(
        "Problem registering BLOSC filter in HDF5 library");
  }
  ...
  H5::DataSet dset(dataGroup.createDataSet("Counts", H5::PredType::NATIVE_INT32,
                                           space, plist));
  ...
  for (offset[0] = 0; offset[0] < _nFrames; offset[0] += count[0]) {
    ...
    dset.write(current_frame.data(), H5::PredType::NATIVE_INT32, memspace,
               space);
  }
  ...
  for (size_t i = 0; i < n_detector_gonio_axes; ++i) {
    ...
    H5::DataSet detectorScan(detectorGroup.createDataSet(
        axis.name(), H5::PredType::NATIVE_DOUBLE, scanSpace));
    detectorScan.write(&values(0), H5::PredType::NATIVE_DOUBLE, scanSpace,
                       scanSpace);
  }
  ...
  for (size_t i = 0; i < n_sample_gonio_axes; ++i) {
    ...
    H5::DataSet sampleScan(sampleGroup.createDataSet(
        axis.name(), H5::PredType::NATIVE_DOUBLE, scanSpace));
    sampleScan.write(&values(0), H5::PredType::NATIVE_DOUBLE, scanSpace,
                     scanSpace);
  }
  ...
  for (const auto &item : map) {
    ...
    try {
      ...
      H5::Attribute intAtt(
          infogroup.createAttribute(item.first, str80, metaSpace));
...
void DataSet::addMask(IMask *mask) { _masks.insert(mask); }
...
const std::set<IMask *> &DataSet::masks() { return _masks; }
...
DataSet::events(const std::vector<ReciprocalVector> &sample_qs) const {
  std::vector<DetectorEvent> events;

  // return true if inside Ewald sphere, false otherwise
  auto compute_sign = [](const Eigen::RowVector3d &q,
                         const InterpolatedState &state) -> bool {
    const Eigen::RowVector3d ki = state.ki().rowVector();
    const Eigen::RowVector3d kf =
        ki + q * state.sampleOrientationMatrix().transpose();
  ...
  for (const ReciprocalVector &sample_q : sample_qs) {
    ...
    Eigen::RowVector3d kf =
        state.ki().rowVector() +
        q_vect * state.sampleOrientationMatrix().transpose();
...
ReciprocalVector DataSet::computeQ(const DetectorEvent &ev) const {
  ...
  const auto *detector = _reader->diffractometer()->detector();
  const auto &detector_position =
      DirectVector(detector->pixelPosition(ev._px, ev._py));
...
std::shared_ptr<IDataReader> DataSet::reader() const { return _reader; }
