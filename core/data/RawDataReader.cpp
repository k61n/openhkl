...
RawDataReader::RawDataReader(const std::string &filename,
                             Diffractometer *diffractometer)
    : IDataReader(filename, diffractometer), _parameters(), _length(0),
      _data() {
...
void RawDataReader::addFrame(const std::string &filename) {
  ...
  int omega, phi, chi;
  for (size_t i = 0, omega = -1, phi = -1, chi = -1; i < n_sample_gonio_axes;
       ++i) {
...
void RawDataReader::swapEndian() {
  ...
  for (unsigned int i = 0; i < _nRows * _nCols; ++i) {
    for (unsigned int byte = 0; byte < _parameters.bpp / 2; ++byte) {
      std::swap(_data[_parameters.bpp * i + byte],
...
Eigen::MatrixXi RawDataReader::data(size_t frame) {
  ...
  if (_length != size_t(file.tellg())) {
    std::string err_msg =
        "data file " + filename + " is not of the expected size: ";
  ...
  default:
    throw std::runtime_error("bpp unsupported: " +
                             std::to_string(_parameters.bpp));
  }
}
