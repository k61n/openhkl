...
private:
    ...
    template <typename U> struct Variant : public Base {

        Variant(const U& value) : Base() { _value = new U {value}; }

        Variant(U& value) : Base() { _value = new U {value}; }

        Variant(U&& value) : Base() { _value = new U {std::forward<U>(value)}; }

        U& get() { return *(reinterpret_cast<U*>(_value)); }

        virtual Base* clone() override { return new Variant(get()); }
...
public:
    Any() : _type_index(typeid(int)), _immutability(false) { _variant = new Variant<int>(0); }

    Any(const Any& other) : _type_index(other._type_index) { _variant = other._variant->clone(); }

    Any(Any& other) : _type_index(other._type_index) { _variant = other._variant->clone(); }
    ...
    template <typename T>
    Any(const T& value, bool immutability = false)
        : _type_index(typeid(T)), _immutability(immutability)
    {
