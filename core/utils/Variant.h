...
template <typename T> struct MaxSizeOf<T> {
  static_assert(!std::is_reference<T>::value,
                "Variant does not support reference types");
...
template <> struct CopyStorage<> {
  CopyStorage(void * /*dst_ptr*/, const std::type_index & /*type*/,
              const void * /*src_ptr*/) {
...
template <> struct MoveStorage<> {
  MoveStorage(void * /*dst_ptr*/, const std::type_index & /*type*/,
              const void * /*src_ptr*/) {
...
public:
  ...
  struct is_subset<T, OtherArgs...> {
    static constexpr bool value =
        IsOneOf<T, Args...>::value && is_subset<OtherArgs...>::value;
  };
  ...
  template <typename T> Variant(T &&value) : _typeid(typeid(T)) {
    ...
    using S = typename std::remove_const<
  ...
  template <typename T> const T &as() const {
    ...
    if (_typeid == typeid(T)) {
      ...
      throw std::runtime_error("error: can not cast Variant from " +
                               std::string(_typeid.name()) + " to " +
