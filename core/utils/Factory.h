...
/**
 * @brief generic factory class templated on the return type, the key and
 * accepts any number and/or type of parameters using variadic templates for the
 * callback.
 */
...
void Factory<base, keytype, args...>::registerCallback(const keytype &key,
                                                       callback cb) {
...
base *Factory<base, keytype, args...>::create(const keytype &key, args... arg) {
  ...
  if (it != _map.end())
    ...
    throw std::invalid_argument("Factory error: callback not registered for " +
                                key + " class");
}
...
std::vector<keytype> Factory<base, keytype, args...>::list() const {
  ...
  std::for_each(_map.begin(), _map.end(),
                [&keys](const std::pair<keytype, callback> &it) {
...
template <typename base, typename keytype, typename... args>
std::size_t
Factory<base, keytype, args...>::unregisterCallback(const keytype &key) {
