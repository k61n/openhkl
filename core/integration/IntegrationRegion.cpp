...
IntegrationRegion::IntegrationRegion() {
  throw std::runtime_error(
      "The default constructor of IntegrationRegion should no be called.");
}

IntegrationRegion::IntegrationRegion(sptrPeak3D peak, double peak_end,
                                     double bkg_begin, double bkg_end)
    : _shape(peak->shape()), _peakEnd(peak_end), _bkgBegin(bkg_begin),
      _bkgEnd(bkg_end), _data(peak) {
...
const AABB IntegrationRegion::aabb() const { return _hull.aabb(); }
...
IntegrationRegion::EventType
IntegrationRegion::classify(const DetectorEvent &ev) const {
...
bool IntegrationRegion::advanceFrame(const Eigen::MatrixXd &image,
...
void IntegrationRegion::reset() { _data.reset(); }

const PeakData &IntegrationRegion::data() const { return _data; }

PeakData &IntegrationRegion::data() { return _data; }

const Ellipsoid &IntegrationRegion::shape() const { return _shape; }

const ConvexHull &IntegrationRegion::hull() const { return _hull; }
