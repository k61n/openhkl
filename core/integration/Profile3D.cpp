...
const Eigen::Vector3d &Profile3D::dx() const { return _dx; }

size_t Profile3D::count() const { return _count; }

size_t Profile3D::addSubdividedValue(const Eigen::Vector3d &x, double y,
                                     size_t subdivide) {
  size_t n = 0;
...
const Eigen::Vector3i &Profile3D::shape() const { return _shape; }
...
void Profile3D::addProfile(const Profile3D &other, double weight) {
  ...
  if (_shape != other._shape) {
    throw std::runtime_error(
        "Profile3D: cannot add profiles of different dimensions");
  }
  ...
  if (dlb.squaredNorm() > 1e-6 * dx || dub.squaredNorm() > 1e-6 * dx) {
    throw std::runtime_error(
        "Profile3D: cannot add profiles with different bounding boxes");
  }
...
const AABB &Profile3D::aabb() const { return _aabb; }
