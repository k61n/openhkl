...
PixelSumIntegrator::PixelSumIntegrator(bool fit_center, bool fit_covariance)
    : MeanBackgroundIntegrator(), _fitCenter(fit_center),
      _fitCovariance(fit_covariance) {}

PixelSumIntegrator::~PixelSumIntegrator() {}

bool PixelSumIntegrator::compute(sptrPeak3D peak,
  ...
  _integratedIntensity =
      Intensity(sum_peak, sum_peak + npeak * mean_bkg +
                              npeak * npeak * std_bkg * std_bkg);
  ...
  Eigen::Matrix3d cov =
      _fitCovariance ? blob.covariance() : peak->shape().inverseMetric();
  ...
  for (int i = 0; i < nframes; ++i) {
    double corrected_intensity =
        intensity_per_frame[i] - n_peak_points_per_frame[i] * mean_bkg;
    _rockingCurve[i] =
        Intensity(corrected_intensity, sqrt(corrected_intensity));
  }
