...
IPeakIntegrator::IPeakIntegrator()
    : _meanBackground(), _integratedIntensity(), _rockingCurve(),
      _handler(nullptr) {}

IPeakIntegrator::~IPeakIntegrator() {}

Intensity IPeakIntegrator::meanBackground() const { return _meanBackground; }
...
void IPeakIntegrator::integrate(PeakList peaks, sptrDataSet data,
  ...
  std::string status =
      "Integrating " + std::to_string(peaks.size()) + " peaks...";
  nsx::info() << status;
  ...
  for (auto peak : peaks) {
    try {
      ...
      regions.emplace(std::make_pair(
          peak, IntegrationRegion(peak, peak_end, bkg_begin, bkg_end)));
    ...
    if (hi[0] >= data->nCols() || hi[1] >= data->nRows() ||
        hi[2] >= data->nFrames()) {
      peak->setSelected(false);
    }
  }
