...
bool MeanBackgroundIntegrator::compute(sptrPeak3D peak,
  ...
  if (events.size() < 20) {
    throw std::runtime_error(
        "MeanBackgroundIntegrator::compute(): too few data points");
  }

  // compute initial mean background and error
  for (auto i = 0; i < counts.size(); ++i) {
    if (region.classify(events[i]) !=
        IntegrationRegion::EventType::BACKGROUND) {
      continue;
    }
    sum_bkg += counts[i];
