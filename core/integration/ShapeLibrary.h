...
PeakList predictPeaks(ShapeLibrary library, sptrDataSet data, sptrUnitCell,
                      double dmin, double dmax, double radius, double nframes,
                      int min_neighbors, PeakInterpolation interpolation);
...
public:
  ...
  ShapeLibrary(bool detector_coords, double peakScale, double bkgBegin,
               double bkgEnd);
  ...
  bool addPeak(sptrPeak3D peak, Profile3D &&profile,
  ...
  Profile3D meanProfile(const DetectorEvent &ev, double radius,
                        double nframes) const;

  //! Return the average peak profile near the given detector event
  std::vector<Intensity> meanProfile1D(const DetectorEvent &ev, double radius,
                                       double nframes) const;

  //! Return the average peak covariance near the given detector event
  Eigen::Matrix3d meanCovariance(sptrPeak3D reference_peak, double radius,
                                 double nframes, size_t min_neighbors,
                                 PeakInterpolation interpolation) const;

  //! Find neighbors of a given peak
  PeakList findNeighbors(const DetectorEvent &ev, double radius,
                         double nframes) const;
