...
static PeakList
buildPeaksFromMillerIndices(sptrDataSet data,
                            const std::vector<MillerIndex> &hkls,
                            const Eigen::Matrix3d &BU) {
...
PeakList predictPeaks(ShapeLibrary library, sptrDataSet data,
                      sptrUnitCell unit_cell, double dmin, double dmax,
  ...
  const auto &mono =
      data->reader()->diffractometer()->source().selectedMonochromator();
  const double wavelength = mono.wavelength();
  auto predicted_hkls =
      unit_cell->generateReflectionsInShell(dmin, dmax, wavelength);

  PeakList peaks = buildPeaksFromMillerIndices(data, predicted_hkls,
                                               unit_cell->reciprocalBasis());
  nsx::info() << "Computing shapes of " << peaks.size()
              << " calculated peaks...";
  ...
  for (auto peak : peaks) {
    ...
    try {
      Eigen::Matrix3d cov = library.meanCovariance(
          peak, radius, nframes, min_neighbors, interpolation);
...
struct FitData {
  ...
  FitData(sptrPeak3D peak) {
    ...
    Jp = Rd * (-1 / r * Eigen::Matrix3d::Identity() +
               1 / r / r / r * dp * dp.transpose());
    Jp *= ki.norm();
...
ShapeLibrary::ShapeLibrary(bool detector_coords, double peakScale,
                           double bkgBegin, double bkgEnd)
    : _profiles(), _choleskyD(), _choleskyM(), _choleskyS(),
      _detectorCoords(detector_coords), _peakScale(peakScale),
      _bkgBegin(bkgBegin), _bkgEnd(bkgEnd) {
  _choleskyD.fill(1e-6);
  _choleskyM.fill(1e-6);
  _choleskyS.fill(1e-6);
}

static void covariance_helper(Eigen::Matrix3d &result, const FitData &f,
                              const Eigen::Matrix3d &sigmaD,
                              const Eigen::Matrix3d &sigmaM,
                              const Eigen::Matrix3d &sigmaS) {
  ...
  Eigen::Matrix3d P =
      f.q.squaredNorm() * Eigen::Matrix3d::Identity() - qs * qs.transpose();
...
double ShapeLibrary::peakScale() const { return _peakScale; }

double ShapeLibrary::bkgBegin() const { return _bkgBegin; }

double ShapeLibrary::bkgEnd() const { return _bkgEnd; }

bool ShapeLibrary::addPeak(sptrPeak3D peak, Profile3D &&profile,
                           Profile1D &&integrated_profile) {
...
double ShapeLibrary::meanPearson() const {
  ...
  for (const auto &pair : _profiles) {
    ...
    sum_pearson += (obs_cov.transpose() * pred_cov).trace() / obs_cov.norm() /
                   pred_cov.norm();
  }
  return sum_pearson / _profiles.size();
}

Profile3D ShapeLibrary::meanProfile(const DetectorEvent &ev, double radius,
...
std::vector<Intensity> ShapeLibrary::meanProfile1D(const DetectorEvent &ev,
                                                   double radius,
                                                   double nframes) const {
  PeakList neighbors = findNeighbors(ev, radius, nframes);
...
PeakList ShapeLibrary::findNeighbors(const DetectorEvent &ev, double radius,
                                     double nframes) const {
  PeakList neighbors;
...
Eigen::Matrix3d
ShapeLibrary::meanCovariance(sptrPeak3D reference_peak, double radius,
                             double nframes, size_t min_neighbors,
                             PeakInterpolation interpolation) const {
  Eigen::Matrix3d cov;
  cov.setZero();
  PeakList neighbors = findNeighbors(
      DetectorEvent(reference_peak->shape().center()), radius, nframes);
  ...
  for (auto peak : neighbors) {
    ...
    case (PeakInterpolation::InverseDistance): {
      Eigen::RowVector3d dq =
          reference_peak->q().rowVector() - peak->q().rowVector();
      weight = 1.0 / dq.norm();
      break;
    }
    ...
    default: { throw std::runtime_error("Invalid peak interpolation"); }
    }
...
bool ShapeLibrary::detectorCoords() const { return _detectorCoords; }
