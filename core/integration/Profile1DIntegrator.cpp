...
Profile1DIntegrator::Profile1DIntegrator(sptrShapeLibrary library,
                                         double radius, double nframes)
    : IPeakIntegrator(), _library(library), _radius(radius), _nframes(nframes) {

}

static void updateFit(Intensity &I, Intensity &B, const std::vector<double> &dp,
...
bool Profile1DIntegrator::compute(sptrPeak3D peak,
  ...
  if (events.size() < 29) {
    throw std::runtime_error(
        "Profile1DIntegrator::compute(): too few data points in peak");
  }
