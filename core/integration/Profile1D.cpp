...
Profile1D::Profile1D(const Intensity &mean_background, double sigma_max,
                     size_t num)
    : _counts(num, 0.0), _npoints(num, 0), _endpoints(num + 1),
      _sigmaMax(sigma_max), _meanBkg(mean_background) {
...
const std::vector<double> &Profile1D::counts() const { return _counts; }

const std::vector<int> &Profile1D::npoints() const { return _npoints; }
