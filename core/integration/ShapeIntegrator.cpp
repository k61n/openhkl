...
ShapeIntegrator::ShapeIntegrator(sptrShapeLibrary lib, const AABB &aabb, int nx,
                                 int ny, int nz)
    : PixelSumIntegrator(false, false), _library(lib), _aabb(aabb), _nx(nx),
      _ny(ny), _nz(nz) {}

bool ShapeIntegrator::compute(sptrPeak3D peak,
  ...
  if (!uc || !data) {
    throw std::runtime_error(
        "ShapeIntegrator: Peak must have unit cell and data attached");
  }
...
sptrShapeLibrary ShapeIntegrator::library() const { return _library; }
