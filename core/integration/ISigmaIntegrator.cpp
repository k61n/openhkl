...
ISigmaIntegrator::ISigmaIntegrator(sptrShapeLibrary library, double radius,
                                   double nframes)
    : PixelSumIntegrator(false, false), _library(library), _radius(radius),
      _nframes(nframes) {}

bool ISigmaIntegrator::compute(sptrPeak3D peak,
  ...
  if (events.size() < 29) {
    throw std::runtime_error(
        "ISigmaIntegrator::compute(): too few data points in peak");
  }
