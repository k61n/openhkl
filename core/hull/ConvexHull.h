...
//!
//! Basically, the algorithm works on the same way than for the 2D case. It works by building
//! counter clockwise oriented faces (triangles) of the hull and at each step a new point is defined
//! to be part of the hull if its signed volume is negative for at least one face. Otherwise the
//! point is discarded. When a point is added to the hull, this one is extended by building new
//! faces that link the point to the edges of the visible faces of the hull from that point. After
//! that step, the hull is updated by removing the faces (and their corresponding edges) that has
//! been completety "buried" by the hull growth.
//!
//! Some of the methods names have been renamed regarding to the C implementation because we felt
//! more comfortable with during those names in the curse of the implementation. In such case the
//! original name will be recalled when documenting those methods.
class ConvexHull {
...
public:
    ...
    //! Adds a new vertex to the list of points to be processed later when calling updateHull
    //! method.
    ...
    //! Updates the hull. The first time it is called two seed-triangles with opposite orientation
    //! will be created on which the next hull faces will be built upon. Original name:
    //! ConstructHull
    void updateHull();
    ...
    //! Returns the volume of the hull. The volume is computed by summing the volumes of all the
    //! tethrahedrons made by each face of the convex hull and any internal point of the hull. A
    //! reasonable choice for the internal point is the center of gravity of the hull as, by
    //! definition of a convex object, its center is within the hull.
    double volume() const;
    ...
    //! Returns the Triangles that builds this ConvexHull if the Hull was to be rotated by the
    //! rotation matrix: rotation Triangles objects are very light object that can be used for
    //! further analysis such as Monte-Carlo based absorption correction.
    std::vector<Triangle>
...
private:
    //! Initializes the hull. The initialization consists in defines two triangles with opposite
    //! orientations that will serve as seeds for the faces of the hull to be built later
    void initalizeHull();
    ...
    //! Builds a new hull face from three vertices. If fold is not nullptr then its edges will be
    //! used to defie the one of the new face to build. Original name: MakeFace
...
private:
    //! A boolean that indicates whether or not the two seed-triangles of the hull have been already
    //! created
    bool _initialized;

    //! The list of the vertices of the hull. This is the std implementation of the double-linked
    //! chain.
    std::map<int, Vertex*> _vertices;

    //! The list of the edges of the hull. This is the std implementation of the double-linked
    //! chain.
    std::map<int, Edge*> _edges;

    //! The list of the faces of the hull. This is the std implementation of the double-linked
    //! chain.
