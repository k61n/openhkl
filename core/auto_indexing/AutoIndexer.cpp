...
void AutoIndexer::computeFFTSolutions() {
  ...
  auto &&tvects =
      indexing.findOnSphere(qvects, _params.nVertices, _params.nSolutions);
  qvects.clear();
    ...
    for (int j = i + 1; j < _params.nSolutions; ++j) {
      for (int k = j + 1; k < _params.nSolutions; ++k) {
        ...
        for (auto solution : _solutions) {
          if (cell->equivalent(*solution.first,
                               _params.unitCellEquivalenceTolerance)) {
            equivalent = true;
            break;
          }
        }
...
void AutoIndexer::refineSolutions() {
  ...
  for (auto &&soln : _solutions) {
    ...
    try {
      ...
      cell->setIndexingTolerance(_params.indexingTolerance);
      cell->reduce(_params.niggliReduction, _params.niggliTolerance,
                   _params.gruberTolerance);
...
void AutoIndexer::addPeak(sptrPeak3D peak) { _peaks.push_back(peak); }
