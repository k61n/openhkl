...
std::vector<FFTIndexing::tVector>
FFTIndexing::findOnSphere(const std::vector<ReciprocalVector> &qvects,
  ...
  size_t nPoints = size_t(std::lround(
      std::ceil(2 * qMax * _nSubdiv * _amax))); // number of points in histogram
  ...
  for (size_t i = 0; i < n_vertices; ++i) {
    ...
    if (pos_max > 2)
      result.push_back(tVector(
          q_direction * (pos_max)*_nSubdiv * _amax / double(nPoints), value));
  }
