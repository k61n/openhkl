...
void UserDefinedUnitCellIndexerParameters::checkParameters() const {
  ...
  if (max_n_q_vectors < 1) {
    throw std::runtime_error(
        "Invalid number of q vectors. Must be in [10,500].");
  }
}

UserDefinedUnitCellIndexer::UserDefinedUnitCellIndexer()
    : _parameters(), _solutions() {}
...
const UserDefinedUnitCellIndexerParameters &
UserDefinedUnitCellIndexer::parameters() const {
  return _parameters;
}
...
void UserDefinedUnitCellIndexer::index() {
  ...
  nsx::UnitCell unit_cell(_parameters.a, _parameters.b, _parameters.c,
                          _parameters.alpha, _parameters.beta,
                          _parameters.gamma);
  ...
  std::vector<std::pair<Eigen::RowVector3d, Eigen::RowVector3d>>
      predicted_q_vectors;
  ...
  std::sort(predicted_q_vectors.begin(), predicted_q_vectors.end(),
  ...
  const double volume_tolerance =
      distance_tolerance * distance_tolerance * distance_tolerance;
  ...
  for (size_t i = 0; i < predicted_q_vectors.size() - 2; ++i) {
    ...
    auto lit_i =
        q_vectors_mmap.lower_bound(qi_norm * (1.0 - distance_tolerance));
    auto uit_i =
        q_vectors_mmap.upper_bound(qi_norm * (1.0 + distance_tolerance));
    ...
    for (size_t j = i + 1; j < predicted_q_vectors.size() - 1; ++j) {
      ...
      auto lit_j =
          q_vectors_mmap.lower_bound(qj_norm * (1.0 - distance_tolerance));
      auto uit_j =
          q_vectors_mmap.upper_bound(qj_norm * (1.0 + distance_tolerance));
      ...
      for (size_t k = j + 1; k < predicted_q_vectors.size(); ++k) {
        ...
        auto lit_k =
            q_vectors_mmap.lower_bound(qk_norm * (1.0 - distance_tolerance));
        auto uit_k =
            q_vectors_mmap.upper_bound(qk_norm * (1.0 + distance_tolerance));
                  ...
                  for (size_t tk = 0; tk < 3; ++tk) {
                    ...
                    for (size_t tii = 0; tii < 3; ++tii) {
                      ...
                      for (size_t tjj = 0; tjj < 3; ++tjj) {
                        ...
                        // Try to match triplet1 and triplet2 j vectors, if the
                        // match fails skip this experimental triplet
                        ...
                        for (size_t tkk = 0; tkk < 3; ++tkk) {
                          ...
                          // Try to match triplet1 and triplet2 k vectors, if
                          ...
                          const double q2iq2j_angle = std::acos(
                              ...
                              q2i_norm / q2j_norm);
                          if (std::fabs((q2iq2j_angle - q1iq1j_angle) /
                                        q1iq1j_angle) > angular_tolerance) {
                            continue;
                          }
                          ...
                          const double q2iq2k_angle = std::acos(
                              ...
                              q2i_norm / q2k_norm);
                          if (std::fabs((q2iq2k_angle - q1iq1k_angle) /
                                        q1iq1k_angle) > angular_tolerance) {
                            continue;
                          }
                          ...
                          const double q2jq2k_angle = std::acos(
                              ...
                              q2j_norm / q2k_norm);
                          if (std::fabs((q2jq2k_angle - q1jq1k_angle) /
                                        q1jq1k_angle) > angular_tolerance) {
                            continue;
                          }
                          ...
                          auto BtUt =
                              matching_hkls.inverse() * matching_q_vectors;
                          ...
                          for (auto q : q_vectors_mmap) {
                            MillerIndex miller_index(
                                nsx::ReciprocalVector(q.second), *unit_cell);
                          ...
                          const double quality =
                              n_indexed_q_vectors /
                              static_cast<double>(q_vectors_mmap.size());
                          if (quality < _parameters.indexing_threshold) {
                            continue;
                          }

                          // The predicted and experimental triplets match, keep
                          // that solution
...
void UserDefinedUnitCellIndexer::removeBadUnitCells() {
  ...
  auto remove_bad_unit_cell = std::remove_if(
      _solutions.begin(), _solutions.end(),
      [=](const indexer_solution &s) { return s.second < quality; });
...
void UserDefinedUnitCellIndexer::refineUnitCells() {
  for (auto &p : _solutions) {
    ...
    try {
      ...
      unit_cell->reduce(_parameters.niggli_only, _parameters.niggli_tolerance,
    ...
    refiltered_peaks = peak_filter.indexed(filtered_peaks, *unit_cell,
                                           unit_cell->indexingTolerance());
...
const std::vector<std::pair<sptrUnitCell, double>> &
UserDefinedUnitCellIndexer::solutions() const {
  return _solutions;
}
