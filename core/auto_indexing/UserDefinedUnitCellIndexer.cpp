...
void UserDefinedUnitCellIndexer::index()
{
    ...
    // The generation uses the initial B matrix built from the unit cell parameters provided by the
    // user hence without orientation matrix
    ...
    for (size_t i = 0; i < predicted_q_vectors.size() - 2; ++i) {
        ...
        for (size_t j = i + 1; j < predicted_q_vectors.size() - 1; ++j) {
            ...
            for (size_t k = j + 1; k < predicted_q_vectors.size(); ++k) {
                ...
                // Triple loop over the experimental q vectors to build experimental q triplets
                // which will be match over the predicted q-triplet (aka qe)
