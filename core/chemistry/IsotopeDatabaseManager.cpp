...
std::map<std::string, ChemicalPropertyType>
    IsotopeDatabaseManager::PropertyTypes = {
...
IsotopeDatabaseManager::IsotopeDatabaseManager() {
  ...
  for (const auto &propertyNode : isotopes_database["Properties"]) {
    std::pair<std::string, std::string> prop =
        std::make_pair(propertyNode.second["type"].as<std::string>(),
                       propertyNode.second["unit"].as<std::string>());
    _properties.insert(
        std::make_pair(propertyNode.first.as<std::string>(), prop));
  }
  ...
  for (const auto &isotopeNode : isotopes_database["Isotopes"]) {
    ...
    for (const auto &propertyNode : isotopeNode.second) {
      ...
      case ChemicalPropertyType::String:
        props.insert(
            std::make_pair(pname, propertyNode.second.as<std::string>()));
        break;
      ...
      case ChemicalPropertyType::Double:
        props.insert(
            std::make_pair(pname, propertyNode.second.as<double>() *
                                      um->get(_properties[pname].second)));
        break;
      case ChemicalPropertyType::Complex:
        props.insert(std::make_pair(
            pname, propertyNode.second.as<std::complex<double>>() *
                       um->get(_properties[pname].second)));
        break;
    ...
    _isotopes.insert(
        std::make_pair(isotopeNode.first.as<std::string>(), props));
  }
}
...
const std::map<std::string, isotopeProperties> &
IsotopeDatabaseManager::isotopes() const {
  return _isotopes;
}
...
bool IsotopeDatabaseManager::hasProperty(const std::string &isotope,
