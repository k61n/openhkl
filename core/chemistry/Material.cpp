...
Material::Material(const std::string &formula)
    : _formula(formula), _isotopes(), _massDensity(1.0) {
  ChemicalFormulaParser<std::string::const_iterator> parser;
  bool success =
      boost::spirit::qi::phrase_parse(formula.begin(), formula.end(), parser,
                                      boost::spirit::qi::blank, _isotopes);
  if (!success) {
    throw std::runtime_error("Invalid input formula");
  }
}

Material *Material::clone() const { return new Material(*this); }

const std::string &Material::formula() const { return _formula; }

const isotopeContents &Material::isotopes() const { return _isotopes; }

double Material::massDensity() const { return _massDensity; }
...
isotopeContents Material::massFractions() const {
  ...
  double molarMass(0.0);
  for (const auto &p : _isotopes) {
    massFractions.insert(std::make_pair(
        p.first, p.second * imgr->property<double>(p.first, "molar_mass")));
...
isotopeContents Material::atomicNumberDensity() const {
  ...
  for (const auto &p : massFractions()) {
    ...
    double molarFraction =
        massFraction / imgr->property<double>(p.first, "molar_mass");
...
double Material::muIncoherent() const {
  ...
  for (const auto &p : atomicNumberDensity()) {
    scatteringMuFactor +=
        p.second * imgr->property<double>(p.first, "xs_incoherent");
  }
...
double Material::muAbsorption(double lambda) const {
  ...
  for (const auto &p : atomicNumberDensity()) {
    ...
    absorptionMuFactor += p.second *
                          imgr->property<double>(p.first, "xs_absorption") *
                          lambda / thermalWavelength;
  }
