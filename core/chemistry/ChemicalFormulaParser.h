        ...
        for (const auto& isotope : isotopeDatabase) {
            ...
            if (symbolName == elementSymbol) {
                output.insert(std::make_pair(
                    isotope.first, imgr->property<double>(isotope.first, "natural_abundance")));
            }
        }
        return true;
    }
...
struct BuildCompoundFromElement {
    bool
    operator()(isotopeContents& output, const isotopeContents& element, double stoichiometry) const
    {
...
//! Functor for building material from a list of chemical compounds and their associated molar ratio
//! in the material. The isotope contents of the material is updated with the isotope contents of
//! the chemical compound multiplied by its molar ratio
...
//! The chemical formula is defined as a string that represents a sequence of elements (e.g. CH4,
//! H2O, C[12]3H[2]8). Each element is in turn defined by a string that represents its chemical
//! symbol as defined in the periodic table respectively followed optionally by the element isotope
//! given in square brackets and by the number of such element in the chemical formula (1 if not
//! provided). The parser is used as usual in spirit given two iterators (begin and end) :
//! parse(begin,end,parser) Example: C3H[2]8 will parse a molecule with three natural  carbons and
//! eight deuterium atoms i.e. a deuterated propane.
