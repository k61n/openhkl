...
//! In such case, the isotope contents of the element is made of that sngle
//! istope and a molar ratio of 1.0
struct BuildElementFromUniqueIsotope {
  bool operator()(isotopeContents &output,
                  const std::string &isotopeName) const {
  ...
  bool operator()(isotopeContents &output,
    ...
    for (const auto &isotope : isotopeDatabase) {
      std::string symbolName =
          imgr->property<std::string>(isotope.first, "symbol");
      if (symbolName == elementSymbol) {
        output.insert(std::make_pair(
            isotope.first,
            imgr->property<double>(isotope.first, "natural_abundance")));
      }
    }
    return true;
  }
...
struct BuildElementFromIsotopeMixture {
  ...
  bool operator()(isotopeContents &output, const std::string &elementSymbol,
    ...
    for (const auto &p : mixture) {
      ...
      std::string isotopeSymbol =
          imgr->property<std::string>(p.first, "symbol");
...
struct BuildCompoundFromElement {
  bool operator()(isotopeContents &output, const isotopeContents &element,
    ...
    for (auto isotope : element) {
      ...
      if (it == output.end()) {
        output.insert(
            std::make_pair(isotope.first, isotope.second * stoichiometry));
      } else {
...
struct BuidMaterialFromCompounds {
  bool operator()(isotopeContents &output,
                  const compoundList &compounds) const {
...
struct ChemicalFormulaParser
    : boost::spirit::qi::grammar<Iterator, isotopeContents()> {
  ChemicalFormulaParser() : ChemicalFormulaParser::base_type(_materialToken) {
    ...
    boost::phoenix::function<BuildElementFromUniqueIsotope>
        build_element_from_unique_isotope;
    // Semantic action for validating the sum of the molar ratio of a set of
    // isotopes
    boost::phoenix::function<ValidateIsotopeContents>
        validate_isotopes_contents;
    // Semantic action for building a chemical element from a mixture of
    // isotopes
    boost::phoenix::function<BuildElementFromIsotopeMixture>
        build_element_from_isotope_mixture;
    ...
    // chemical element
    boost::phoenix::function<BuildCompoundFromElement>
        build_compound_from_element;

    // Semantic action for validating the sum of the molar ratio of a list of
    // compounds
    boost::phoenix::function<ValidateCompoundContents>
        validate_compounds_contents;
    // Semantic action for building a material from a list of chemical compounds
    boost::phoenix::function<BuidMaterialFromCompounds>
        build_material_from_compounds;
    ...
    _isotopeMixtureToken =
        (_elementToken >>
         _mixtureToken)[_pass =
                            build_element_from_isotope_mixture(_val, _1, _2)];
    _uniqueIsotopeToken =
        (_isotopeToken)[_pass = build_element_from_unique_isotope(_val, _1)];
    ...
    _compoundToken = +(
        ((_isotopeMixtureToken | _uniqueIsotopeToken | _naturalElementToken) >>
         (double_ |
          attr(1.0)))[_pass = build_compound_from_element(_val, _1, _2)]);
    ...
    _materialToken =
        eps > ((_compoundMixtureToken[_pass = build_material_from_compounds(
                                          _val, _1)]) > eoi);
  }
