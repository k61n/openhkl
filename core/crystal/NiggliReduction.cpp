...
NiggliReduction::NiggliReduction(const Eigen::Matrix3d &g, double epsilon)
  ...
  if (epsilon <= 0 || epsilon > 1)
    throw std::runtime_error(
        "Niggli reduction: epsilon must be in the range ]0,1]");
...
void NiggliReduction::reduce(Eigen::Matrix3d &newg, Eigen::Matrix3d &P) {
  ...
  for (unsigned int i = 0; i < _itermax; ++i) {
    ...
    // Step 1
    if ((_A > _B + _epsilon) || (!(std::abs(_A - _B) > _epsilon) &&
                                 (std::abs(_xi) > std::abs(_eta) + _epsilon))) {
      _CMat << 0, -1, 0, -1, 0, 0, 0, 0, -1;
      transformG();
      updateParameters();
    }
    // Step 2
    if (_B > _C + _epsilon || (!(std::abs(_B - _C) > _epsilon) &&
                               (std::abs(_eta) > std::abs(_zeta) + _epsilon))) {
    ...
    cond2 =
        !(std::abs(_B - _xi) > _epsilon) && ((2 * _eta) < (_zeta - _epsilon));
    ...
    cond2 =
        !(std::abs(_A - _eta) > _epsilon) && ((2 * _xi) < (_zeta - _epsilon));
    ...
    cond2 =
        !(std::abs(_A - _zeta) > _epsilon) && ((2 * _xi) < (_eta - _epsilon));
