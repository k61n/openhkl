 ...
 **/
void remove_duplicates(std::vector<Eigen::RowVector3d> &q_vectors, bool reflect,
                       double eps) {
  ...
  for (const auto v : q_vectors) {
    ...
    for (const auto w : unique_q_vectors) {
      if ((v - w).squaredNorm() < eps2 ||
          (reflect && (v + w).squaredNorm() < eps2)) {
        duplicate = true;
        break;
      }
    }
...
void BrillouinZone::clean_qs() {
  ...
  std::remove_if(_qs.begin(), _qs.end(),
                 [&](const Eigen::RowVector3d &v) { return !inside(0.5 * v); });
