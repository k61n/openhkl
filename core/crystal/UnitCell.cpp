...
UnitCell UnitCell::interpolate(const UnitCell &uc1, const UnitCell &uc2,
                               double t) {
...
UnitCellCharacter::UnitCellCharacter()
    : g00(0.0), g01(0.0), g02(0.0), g11(0.0), g12(0.0), g22(0.0), a(0.0),
      b(0.0), c(0.0), alpha(0.0), beta(0.0), gamma(0.0) {}

UnitCellCharacter::UnitCellCharacter(const Eigen::Matrix3d &g)
    : UnitCellCharacter(g(0, 0), g(0, 1), g(0, 2), g(1, 1), g(1, 2), g(2, 2)) {}

UnitCellCharacter::UnitCellCharacter(double g00_, double g01_, double g02_,
                                     double g11_, double g12_, double g22_) {
  g00 = g00_;
  g01 = g01_;
  g02 = g02_;
  g11 = g11_;
...
UnitCell::UnitCell()
    : _a(Eigen::Matrix3d::Identity()),
      ...
      _NP(Eigen::Matrix3d::Identity()), _material(nullptr),
      _centring(LatticeCentring::P), _bravaisType(BravaisType::Triclinic),
      _Z(1), _space_group("P 1"), _name("uc"), _indexingTolerance(0.2),
      _niggli() {}

UnitCell::UnitCell(const UnitCell &other)
    : _a(other._a), _b_transposed(other._b_transposed), _NP(other._NP),
      _centring(other._centring), _bravaisType(other._bravaisType),
      _Z(other._Z), _space_group(other._space_group), _name(other._name),
      _indexingTolerance(other._indexingTolerance), _niggli(other._niggli),
      _characterSigmas(other._characterSigmas), _states(other._states) {
...
void UnitCell::setState(sptrDataSet data, size_t frame,
                        const UnitCellState &state) {
  auto it = _states.find(data);
  if (it == _states.end()) {
    initState(data);
  }
...
UnitCell UnitCell::interpolate(sptrDataSet data, double frame) {
  ...
  if (frame > (n_frames - 1) || frame < 0) {
    throw std::runtime_error(
        "Error when interpolating state: invalid frame value: " +
        std::to_string(frame));
  }
...
void UnitCell::setParameters(double a, double b, double c, double alpha,
...
void UnitCell::setReciprocalParameters(double as, double bs, double cs,
  ...
  const double metric_factor = std::sqrt(1.0 - cas * cas - cbs * cbs -
                                         cgs * cgs + 2.0 * cas * cbs * cgs);

  const double c = sgs / metric_factor / cs;

  _b_transposed << as, bs * cgs, cs * cbs, 0, bs * sgs, -cs * sbs * ca, 0, 0,
      1 / c;

  _a = _b_transposed.inverse();
}

void UnitCell::setMetric(double g00, double g01, double g02, double g11,
...
void UnitCell::setBravaisType(BravaisType bravais) { _bravaisType = bravais; }
...
void UnitCell::printSelf(std::ostream &os) const {
  ...
  os << std::fixed << std::setw(10) << std::setprecision(5) << c.gamma / deg
     << std::endl;
  ...
  os << std::fixed << std::setw(10) << std::setprecision(5) << rc.gamma / deg
     << std::endl;
  ...
  if (_material) {
    ...
    os << "Linear absorption coef: " << _material->muAbsorption() * cm
       << "cm-1 @ 1.798 AA \n";
    os << "Linear incoherent coef: " << _material->muIncoherent() * cm
       << "cm-1";
  }
}
...
UnitCell::generateReflectionsInShell(double dmin, double dmax,
  ...
  const int hkl_max =
      std::ceil(2.0 / (wavelength * b_min)); // maximum allowed by Bragg law
  ...
  nsx::info() << "Generated " << hkls.size() << " hkl in [" << dmin << ","
              << dmax << "] range";

  return hkls;
}

double UnitCell::angle(const Eigen::RowVector3d &hkl1,
                       const Eigen::RowVector3d &hkl2) const {
...
unsigned int UnitCell::z() const { return _Z; }

void UnitCell::setZ(unsigned int Z) { _Z = Z; }

Material *UnitCell::material() { return _material.get(); }

const Material *UnitCell::material() const { return _material.get(); }
...
const std::string &UnitCell::name() const { return _name; }
...
double UnitCell::indexingTolerance() const { return _indexingTolerance; }

void UnitCell::setNiggli(const NiggliCharacter &niggli) { _niggli = niggli; }
...
UnitCell UnitCell::applyNiggliConstraints() const {
  ...
  const double delta =
      (new_uc.reciprocalBasis() - _b_transposed).norm() / _b_transposed.norm();

  if (delta < 0.1) {
    return new_uc;
  }
  throw std::runtime_error(
      "ERROR: could not apply symmetry constraints to unit cell");
}
...
UnitCellCharacter UnitCell::characterSigmas() const { return _characterSigmas; }

UnitCellCharacter UnitCell::reciprocalCharacter() const {
  return UnitCellCharacter(reciprocalMetric());
}

double UnitCell::volume() const { return std::fabs(_a.determinant()); }

Eigen::Matrix3d UnitCell::metric() const { return _a.transpose() * _a; }

Eigen::Matrix3d UnitCell::reciprocalMetric() const {
  return _b_transposed * _b_transposed.transpose();
}

const Eigen::Matrix3d &UnitCell::basis() const { return _a; }
...
int UnitCell::reduce(bool niggli_only, double niggliTolerance,
                     double gruberTolerance) {
...
const NiggliCharacter &UnitCell::niggliCharacter() const { return _niggli; }

Eigen::Matrix3d UnitCell::niggliBasis() const { return _a * _NP.inverse(); }
...
const Eigen::Matrix3d &UnitCell::niggliTransformation() const { return _NP; }
...
UnitCell UnitCell::fromParameters(const Eigen::Matrix3d &U0,
                                  const Eigen::Vector3d &uOffset,
