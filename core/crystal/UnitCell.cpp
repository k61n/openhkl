...
//! symmetry constraints on the Niggli character (see e.g. tables 9.2.5.1 of volume A of the
//! international tables). Since the constraints are _non-linear_ on the components of the basis
//! matrix, we perform a change of coordinates into a system where the constraints are linear.
//!
//! We parameterize the basis matrix as U1 * U0 * R, where U1 is rotation, U0 is the orientation of
//! the initial guess, and R is upper triangular. The upper triangular matrix R is parameterized by
//! the 6 lattice character components A, B, C, D, E, F on which the constraints are linear. We
//! construct the rotation matrix U1 from a unit quaternion, which is parameterized by 3 real
//! numbers. So the total number of parameters is 3 quaternion components + 6 lattice characters - #
//! constraints.
//!
//! We further note that the factorization U = U1 * U0 is done for numerical stability (since we
//! expect U1 to be small) and that we use quaternions to compute U1 also for numerical stability
//! (and furthermore to guarantee that U1 is unitary, which is a non-linear constraint on its
//! components).
//!
//! Given this parameterization and initial basis matrices A0, B0 = A0^{-1}, we perform a
//! least-squares minimization to find A, B = A^{-1} such that the L2 matrix norm |B-B0|^2 is
//! minimized. The reason for choosing |B-B0|^2 as the objective function is so that the resulting
//! predicted q values will be as close as possible.
