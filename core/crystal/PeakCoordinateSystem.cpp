...
PeakCoordinateSystem::PeakCoordinateSystem(sptrPeak3D peak) : _peak(peak) {
  if (!_peak) {
    throw std::runtime_error(
        "Cannot construct PeakCoordinateSystem from null Peak3d");
  }

  _event = DetectorEvent(peak->shape().center());
  _state = peak->data()->interpolatedState(_event._frame);
  _ki = _state.ki().rowVector();
  _kf = peak->q().rowVector() * _state.sampleOrientationMatrix().transpose() +
        _ki;
...
// Kabsh coordinate system
  ...
  const double eps3 = _kf.dot(dq) / _kf.norm() / (_kf - _ki).norm() *
                      (ev._frame - _event._frame);
#endif
...
Ellipsoid PeakCoordinateSystem::detectorShape(double sigmaD,
                                              double sigmaM) const {
