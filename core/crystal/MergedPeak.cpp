...
MillerIndex MergedPeak::index() const { return _hkl; }

Intensity MergedPeak::intensity() const {
  return _intensitySum / _peaks.size();
}

size_t MergedPeak::redundancy() const { return _peaks.size(); }
...
void MergedPeak::determineRepresentativeHKL() {
  ...
  auto compare_fn = [=](const Eigen::RowVector3d &a,
                        const Eigen::RowVector3d &b) -> bool {
...
const PeakList &MergedPeak::peaks() const { return _peaks; }
