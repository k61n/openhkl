...
SymOp::SymOp(std::string generator) { _matrix = parseJonesSymbol(generator); }

SymOp::SymOp(const affineTransformation &symmetryOperation)
    : _matrix(symmetryOperation) {}
...
const affineTransformation &SymOp::getMatrix() const { return _matrix; }
...
void SymOp::print(std::ostream &os) const { os << getJonesSymbol(); }
...
bool SymOp::hasTranslation() const {
  return (std::abs(_matrix(0, 3)) > 1e-3 || std::abs(_matrix(1, 3)) > 1e-3 ||
          std::abs(_matrix(2, 3)) > 1e-3);
}

bool SymOp::isPureTranslation() const {
  return (_matrix.linear().isIdentity(1.0e-3) &&
          !_matrix.translation().isZero(1.0e-3));
}
...
Eigen::Matrix3d SymOp::getRotationPart() const { return _matrix.linear(); }
