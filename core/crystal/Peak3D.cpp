...
void Peak3D::setShape(const Ellipsoid &shape) {
  ...
  if (_data) {
    ...
    if (c[2] < 0.0 || c[2] > _data->nFrames() - 1 || c[0] < 0.0 ||
        c[0] > _data->nCols() - 1 || c[1] < 0.0 || c[1] > _data->nRows() - 1) {
...
const Ellipsoid &Peak3D::shape() const { return _shape; }

const std::vector<Intensity> &Peak3D::rockingCurve() const {
  return _rockingCurve;
}

void Peak3D::setUnitCell(sptrUnitCell uc) { _unitCell = uc; }

sptrUnitCell Peak3D::unitCell() const { return _unitCell; }

Intensity Peak3D::rawIntensity() const { return _rawIntensity; }
...
double Peak3D::transmission() const { return _transmission; }

double Peak3D::scale() const { return _scale; }

void Peak3D::setScale(double factor) { _scale = factor; }

void Peak3D::setTransmission(double transmission) {
  _transmission = transmission;
}

bool Peak3D::enabled() const { return (!_masked && _selected); }

void Peak3D::setSelected(bool s) { _selected = s; }

bool Peak3D::selected() const { return _selected; }

void Peak3D::setMasked(bool masked) { _masked = masked; }

bool Peak3D::masked() const { return _masked; }

void Peak3D::setPredicted(bool predicted) { _predicted = predicted; }

bool Peak3D::predicted() const { return _predicted; }
...
Ellipsoid Peak3D::qShape() const {
  if (!_data) {
    throw std::runtime_error(
        "Attempted to compute q-shape of peak not attached to data");
  }
...
ReciprocalVector Peak3D::qPredicted() const {
  ...
  return ReciprocalVector(
      _unitCell->fromIndex(index.rowVector().cast<double>()));
}
...
DetectorEvent Peak3D::predictCenter(double frame) const {
  ...
  Eigen::RowVector3d q_hkl =
      _unitCell->fromIndex(index.rowVector().cast<double>());
  Eigen::RowVector3d ki = state.ki().rowVector();
  Eigen::RowVector3d kf =
      q_hkl * state.sampleOrientationMatrix().transpose() + ki;
  ...
  Eigen::RowVector3d pred_kf =
      (kf1 - kf).norm() < (kf2 - kf).norm() ? kf1 : kf2;
...
Intensity Peak3D::meanBackground() const { return _meanBackground; }

double Peak3D::peakEnd() const { return _peakEnd; }

double Peak3D::bkgBegin() const { return _bkgBegin; }

double Peak3D::bkgEnd() const { return _bkgEnd; }
