...
parseJonesSymbol(const std::string &jonesSymbol) {
  ...
  auto check_white_space = [](unsigned char const c) {
  ...
  for (size_t i = 0; i < symbols.size(); ++i) {
    ...
    auto rotation_expr_begin =
        std::sregex_iterator(symbol.begin(), symbol.end(), rotation_re,
    ...
    if (!suffix.empty()) {
      ...
      bool success =
          std::regex_match(suffix, translation_match, translation_re);
