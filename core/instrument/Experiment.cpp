...
Experiment::Experiment(const std::string &name,
                       const std::string &diffractometerName)
    : _name(name), _data() {
...
Diffractometer *Experiment::diffractometer() { return _diffractometer.get(); }
...
sptrDataSet Experiment::data(std::string name) {
  ...
  if (it == _data.end()) {
    throw std::runtime_error("The data " + name +
                             " could not be found in the experiment " + _name);
  }
  return it->second;
}

const std::string &Experiment::name() const { return _name; }

void Experiment::setName(const std::string &name) { _name = name; }
