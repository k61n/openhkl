...
Sample *Sample::create(const YAML::Node &node) { return new Sample(node); }
...
void Sample::setShape(const ConvexHull &shape) { _sampleShape = shape; }

ConvexHull &Sample::shape() { return _sampleShape; }

const ConvexHull &Sample::shape() const { return _sampleShape; }

SampleGonioFit Sample::fitGonioOffsets(const DataList &dataset,
                                       size_t n_iterations,
  ...
  auto residuals = [sample_gonio, &fitted_offsets, selected_orientations,
                    selected_states,
    ...
    for (int i = 0; i < n_obs; ++i) {
      ...
      std::transform(state.begin(), state.end(), fitted_offsets.begin(),
                     real_values.begin(), std::plus<double>());
