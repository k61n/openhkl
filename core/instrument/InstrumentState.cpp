...
ReciprocalVector
InstrumentState::kfLab(const DirectVector &detector_position) const {
  Eigen::Vector3d k =
      detectorOrientation * (detector_position.vector() - samplePosition);
  k.normalize();
  k /= wavelength;
...
ReciprocalVector
InstrumentState::sampleQ(const DirectVector &detector_position) const {
...
Eigen::Matrix3d InstrumentState::sampleOrientationMatrix() const {
  return (sampleOrientationOffset * sampleOrientation)
      .normalized()
      .toRotationMatrix();
}
...
Eigen::Matrix3d InstrumentState::jacobianK(double px, double py) const {
  ...
  Eigen::Matrix3d dkdx =
      nki * detectorOrientation * (dpdx / r - dp * drdx / r / r);
...
Diffractometer *InstrumentState::diffractometer() { return _diffractometer; }
