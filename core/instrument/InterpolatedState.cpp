...
InterpolatedState::InterpolatedState(const InstrumentState &s1,
                                     const InstrumentState &s2, double t)
    : InstrumentState(const_cast<Diffractometer *>(s1.diffractometer())),
      axis(), stepSize() {
  if (s1.diffractometer() != s2.diffractometer()) {
    throw std::runtime_error(
        "Cannot interpolate states between different diffractometers");
  }

  const double s = 1 - t;

  detectorOrientation =
      interpolateRotation(s1.detectorOrientation, s2.detectorOrientation, t);
  detectorPositionOffset =
      s * s1.detectorPositionOffset + t * s2.detectorPositionOffset;
...
double InterpolatedState::lorentzFactor(double px, double py) const {
  auto position = _diffractometer->detector()->pixelPosition(px, py);
  Eigen::Vector3d q0 = sampleQ(position).rowVector();
  Eigen::Vector3d kf = sampleOrientationMatrix().transpose() *
                       kfLab(position).rowVector().transpose();
