...
Source::Source()
    : Component("source"), _monochromators(), _selectedMonochromator(0) {}

Source::Source(const Source &other)
    : Component(other), _monochromators(other._monochromators),
...
Source::Source(const YAML::Node &node)
    : Component(node), _selectedMonochromator(0) {
...
int Source::nMonochromators() const { return _monochromators.size(); }
...
void Source::setSelectedMonochromator(size_t i) {
  if (i < _monochromators.size()) {
    ...
    throw std::runtime_error(
        "setSelectedMonochromator(): index i is out of range");
  }
}
