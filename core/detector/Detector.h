...
public:
  ...
  //! Return the detector event (pixel x, pixel y, time of flight) associated
  //! with a given kf. Returns with _negative_ tof if no such event is possible.
  virtual DetectorEvent constructEvent(const DirectVector &from,
                                       const ReciprocalVector &kf) const = 0;
  ...
  //! Return the Jacobian matrix of the transformation (px,py) -> (x,y,z) from
  ...
  //! Return the detector baseline. Measured count = gain * (neutron count) +
  //! baseline
  double baseline() const;
  //! Return the detector gain. Measured count = gain * (neutron count) +
  //! baseline
  double gain() const;

  DetectorGonioFit fitGonioOffsets(const DataList &dataset,
