...
Detector::Detector()
    : Component("detector"), _height(0.0), _width(0.0), _angularHeight(0.0),
      _angularWidth(0.0), _nRows(0), _nCols(0), _minRow(0.0), _minCol(0.0),
      _distance(0), _dataorder(DataOrder::BottomLeftColMajor), _baseline(0.0),
      _gain(1.0) {}

Detector::Detector(const std::string &name)
    : Component(name), _height(0.0), _width(0.0), _angularHeight(0.0),
      _angularWidth(0.0), _nRows(0), _nCols(0), _minRow(0.0), _minCol(0.0),
      _distance(0), _dataorder(DataOrder::BottomLeftColMajor), _baseline(0.0),
      _gain(1.0) {}
...
DataOrder Detector::dataOrder() const { return _dataorder; }

double Detector::baseline() const { return _baseline; }

double Detector::gain() const { return _gain; }

double Detector::height() const { return _height; }

double Detector::width() const { return _width; }

int Detector::minRow() const { return _minRow; }

int Detector::maxRow() const { return _minRow + _nRows; }

int Detector::minCol() const { return _minCol; }

int Detector::maxCol() const { return _minCol + _nCols; }

double Detector::angularHeight() const { return _angularHeight; }

double Detector::angularWidth() const { return _angularWidth; }

double Detector::distance() const { return _distance; }
...
bool Detector::hasPixel(double px, double py) const {
  ...
  return (dx >= 0 && dx < static_cast<double>(_nCols) && dy >= 0 &&
          dy < static_cast<double>(_nRows));
}

unsigned int Detector::nCols() const { return _nCols; }

void Detector::setNCols(unsigned int cols) {
  if (cols == 0) {
    throw std::range_error("Detector " + Component::_name +
                           " number of pixels (row,col) must be >0");
  }
  _nCols = cols;
}

unsigned int Detector::nRows() const { return _nRows; }
...
double Detector::pixelHeight() const { return _height / _nRows; }

double Detector::pixelWidth() const { return _width / _nCols; }

DetectorGonioFit Detector::fitGonioOffsets(const DataList &dataset,
                                           size_t n_iterations,
  ...
  auto residuals = [detector_gonio, &fitted_offsets, selected_nis,
    ...
    for (int i = 0; i < n_obs; ++i) {
      f(i) = std::abs(selected_nis[i].dot(Eigen::RowVector3d(0, 1, 0) *
                                          fitted_detector_orientation) -
                      1.0);
    }
