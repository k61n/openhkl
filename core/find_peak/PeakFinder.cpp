...
void PeakFinder::findPrimaryBlobs(
    const DataSet& dataset, std::map<int, Blob3D>& blobs, EquivalenceList& equivalences,
...
#pragma omp critical
    ...
    // Labels of the left and top pixels with respect to current one and the one above in previous
    // frame
...
void PeakFinder::findCollisions(
    const DataSet& dataset, std::map<int, Blob3D>& blobs, EquivalenceList& equivalences) const
{
