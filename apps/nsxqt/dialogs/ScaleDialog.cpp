...
void ScaleDialog::buildPlot()
{
    ...
    for (auto&& peak_list: _peaks) {
        ...
        for (auto&& p: peak_list) {
            double frame = p->getRegion().getPeak().getAABBCenter()[2];
        ...
        for (auto&& p: peak_list) {
            ...
            double x = p->getRegion().getPeak().getAABBCenter()[2]; // frame
...
void ScaleDialog::calculateRFactors()
{
    ...
    for (size_t i = 0; i < _peaks.size(); ++i) {
        ...
        for (auto&& p: peak_list) {
            double z = p->getRegion().getPeak().getAABBCenter()[2];
...
void ScaleDialog::setScale()
{
    for (auto& peak_list: _peaks) {
        for (auto& peak: peak_list) {
            double z = peak->getRegion().getPeak().getAABBCenter()[2];
