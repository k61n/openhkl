...
void RulerItem::moveTipAt(const QPointF& pos)
{
    ...
    // Set the three lines
    _base->setLine(_start.x()+sangle,_start.y()-cangle,_start.x()-sangle,_start.y()+cangle);
    ...
    if (_displaydistance && _calibrated)
    {
        ...
        _bodytext->setPos(_start.x()+0.5*dd*cangle/_hwidth+8,_start.y()+0.5*dd*sangle/_hwidth+12);
