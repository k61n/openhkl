...
CutSliceGraphicsItem::CutSliceGraphicsItem(nsx::sptrDataSet data,
                                           bool horizontal)
...
void CutSliceGraphicsItem::plot(SXPlot *plot) {
  ...
  std::transform(y.begin(), y.end(), e.begin(),
                 [](double p) { return sqrt(p); });
  p->graph(0)->setDataValueError(x, y, e);
  p->rescaleAxes();
  p->replot(QCustomPlot::rpHint);
}

bool CutSliceGraphicsItem::isHorizontal() const { return _horizontal; }

void CutSliceGraphicsItem::paint(QPainter *painter,
...
std::string CutSliceGraphicsItem::getPlotType() const { return "simple"; }
