...
nsx::sptrPeak3D PeakGraphicsItem::peak() const { return _peak; }
...
void PeakGraphicsItem::paint(QPainter *painter,
...
std::string PeakGraphicsItem::getPlotType() const { return "peak"; }

void PeakGraphicsItem::showLabel(bool flag) { _show_label = flag; }

void PeakGraphicsItem::showArea(bool flag) { _show_center = flag; }
...
void PeakGraphicsItem::plot(SXPlot *plot) {
  ...
  if (auto cell = _peak->unitCell()) {
    ...
    if (miller_index.indexed(cell->indexingTolerance())) {
      info = "(h,k,l):" + QString::number(miller_index[0]) + "," +
             QString::number(miller_index[1]) + "," +
             QString::number(miller_index[2]);
    } else {
      info = "unindexed";
    }
  } else {
  ...
  auto position =
      _peak->data()->reader()->diffractometer()->detector()->pixelPosition(
          c[0], c[1]);
  ...
  info += "Intensity (" + QString(QChar(0x03C3)) +
          "I): " + QString::number(intensity) + " (" +
  ...
  QCPPlotTitle *title =
      dynamic_cast<QCPPlotTitle *>(p->plotLayout()->element(0, 0));
  if (title != nullptr) {
    title->setText(info);
  }
  p->rescaleAxes();
