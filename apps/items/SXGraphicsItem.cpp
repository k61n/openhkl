...
SXGraphicsItem::SXGraphicsItem(QGraphicsItem *parent, bool deletable,
                               bool movable)
    : QGraphicsItem(parent), _deletable(deletable), _hovered(false),
      _movable(movable), _firstMove(true), _lastPos(), _label_gi(nullptr) {
  _pen.setWidth(1);
...
void SXGraphicsItem::setDeletable(bool deletable) { _deletable = deletable; }
...
bool SXGraphicsItem::isDeletable() const { return _deletable; }

bool SXGraphicsItem::isMovable() const { return _movable; }

void SXGraphicsItem::showLabel(bool show) { _label_gi->setVisible(show); }
