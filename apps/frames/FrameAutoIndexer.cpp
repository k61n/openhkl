...
FrameAutoIndexer::FrameAutoIndexer(ExperimentItem* experiment_item, const nsx::PeakList& peaks)
    : NSXQFrame(), _ui(new Ui::FrameAutoIndexer), _experiment_item(experiment_item)
{
    _ui->setupUi(this);
    ...
    for (auto peak : peaks) {
        ...
        if (unit_cell) {
            _defaults.push_back(
                std::make_pair(peak, std::make_shared<nsx::UnitCell>(nsx::UnitCell(*unit_cell))));
        } else {
    ...
    connect(
        _ui->solutions->verticalHeader(), SIGNAL(sectionDoubleClicked(int)), this,
        SLOT(selectSolution(int)));
...
void FrameAutoIndexer::slotTabEdited(int index)
{
    ...
    QModelIndex bottomright_index = _peaks_model->index(
        _peaks_model->rowCount(QModelIndex()) - 1, _peaks_model->columnCount(QModelIndex()) - 1);
...
void FrameAutoIndexer::resetUnitCell()
{
    ...
    QModelIndex bottomRight = _peaks_model->index(
        _peaks_model->rowCount(QModelIndex()) - 1, _peaks_model->columnCount(QModelIndex()) - 1);
