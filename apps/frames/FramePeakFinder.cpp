...
FramePeakFinder *FramePeakFinder::create(ExperimentItem *experiment_item,
                                         const nsx::DataList &data) {
  if (!_instance) {
...
FramePeakFinder *FramePeakFinder::Instance() { return _instance; }

FramePeakFinder::FramePeakFinder(ExperimentItem *experiment_item,
                                 const nsx::DataList &data)
    : NSXQFrame(), _ui(new Ui::FramePeakFinder),
      _experiment_item(experiment_item), _pixmap(nullptr),
      _colormap(new ColorMap) {
  _ui->setupUi(this);

  _ui->tabs->tabBar()->tabButton(0, QTabBar::RightSide)->hide();

  DoubleItemDelegate *convolution_parameters_delegate =
      new DoubleItemDelegate();
  _ui->convolution_parameters->setItemDelegateForColumn(
  ...
  for (auto &&convolution_kernel : convolver_factory.callbacks()) {
    _ui->convolution_kernels->addItem(
  ...
  connect(_ui->min_blob_size, SIGNAL(valueChanged(int)), this,
          SLOT(changeMinBlobSize()));
  connect(_ui->max_blob_size, SIGNAL(valueChanged(int)), this,
          SLOT(changeMaxBlobSize()));

  // The integration parameters update
  connect(_ui->peak_scale, SIGNAL(valueChanged(double)), this,
          SLOT(changePeakScale()));
  connect(_ui->background_begin_scale, SIGNAL(valueChanged(double)), this,
          SLOT(changeBackgroundBegin()));
  connect(_ui->background_end_scale, SIGNAL(valueChanged(double)), this,
          SLOT(changeBackgroundEnd()));

  connect(_ui->convolution_kernels, SIGNAL(currentTextChanged(QString)), this,
          SLOT(changeConvolutionKernel(QString)));

  // The change of data to preview
  connect(_ui->selected_data, SIGNAL(currentIndexChanged(int)), this,
          SLOT(changePreviewData()));

  connect(_ui->apply_threshold, SIGNAL(stateChanged(int)), this,
          SLOT(clipPreview(int)));

  connect(_ui->selected_frame, SIGNAL(valueChanged(int)), this,
          SLOT(changeSelectedFrame(int)));
  connect(_ui->selected_frame_slider, SIGNAL(valueChanged(int)), this,
          SLOT(changeSelectedFrame(int)));
  ...
  connect(_ui->actions, SIGNAL(clicked(QAbstractButton *)), this,
          SLOT(doActions(QAbstractButton *)));

  emit _ui->selected_data->currentIndexChanged(
      _ui->selected_data->currentIndex());
}
...
void FramePeakFinder::slotTabEdited(int index) {
  auto found_peaks_tab =
      dynamic_cast<WidgetFoundPeaks *>(_ui->tabs->widget(index));
...
void FramePeakFinder::accept() {
  ...
  for (auto i = 0; i < _ui->tabs->count(); ++i) {
    ...
    auto widget_found_peaks =
        dynamic_cast<WidgetFoundPeaks *>(_ui->tabs->widget(i));
    ...
    auto checkbox = dynamic_cast<QCheckBox *>(
        _ui->tabs->tabBar()->tabButton(i, QTabBar::LeftSide));
...
void FramePeakFinder::run() {
  nsx::info() << "Peak find algorithm: Searching peaks in "
              << _ui->selected_data->count() << " files";
  ...
  for (int i = 0; i < _ui->selected_data->count(); ++i) {
    data.push_back(_ui->selected_data->itemData(i, Qt::UserRole)
                       .value<nsx::sptrDataSet>());
  }
  ...
  std::string convolver_type =
      _ui->convolution_kernels->currentText().toStdString();
  ...
  // execute in a try-block because the progress handler may throw if it is
  // aborted by GUI
  try {
  ...
  for (auto d : data) {
    ...
    integrator.integrate(peaks, d, _ui->peak_scale->value(),
                         _ui->background_begin_scale->value(),
                         _ui->background_end_scale->value());
  }

  WidgetFoundPeaks *widget_found_peaks =
      new WidgetFoundPeaks(_experiment_item, peaks);
...
std::map<std::string, double> FramePeakFinder::convolutionParameters() const {
  ...
  for (int i = 0; i < _ui->convolution_parameters->rowCount(); ++i) {
    std::string pname =
        _ui->convolution_parameters->item(i, 0)->text().toStdString();
...
void FramePeakFinder::updateConvolutionParameters() {
  auto convolution_kernel_name =
      _ui->convolution_kernels->currentText().toStdString();

  nsx::ConvolverFactory convolution_kernel_factory;

  auto convolution_kernel =
      convolution_kernel_factory.create(convolution_kernel_name, {});

  // Get its corresponding parameters
  const std::map<std::string, double> &parameters =
      convolution_kernel->parameters();

  disconnect(_ui->convolution_parameters, SIGNAL(cellChanged(int, int)), this,
             SLOT(changeConvolutionParameters(int, int)));
  ...
  connect(_ui->convolution_parameters, SIGNAL(cellChanged(int, int)), this,
          SLOT(changeConvolutionParameters(int, int)));
}

void FramePeakFinder::changePreviewData() {
  auto data =
      _ui->selected_data->currentData(Qt::UserRole).value<nsx::sptrDataSet>();
...
void FramePeakFinder::preview() {
  ...
  Eigen::MatrixXd convolved_frame = data->convolvedFrame(
      selected_frame, convolver_type, convolver_parameters);
  ...
  QImage image = _colormap->matToImage(convolved_frame.cast<double>(), rect,
                                       convolved_frame.maxCoeff());
...
void FramePeakFinder::changePeakScale() {
  ...
  _ui->peak_scale->setValue(
      std::min(value, _ui->background_begin_scale->value()));
}
...
void FramePeakFinder::changeBackgroundBegin() {
  ...
  _ui->background_begin_scale->setValue(
      std::min(value, _ui->background_end_scale->value()));
}

void FramePeakFinder::changeBackgroundEnd() {
  auto &&value = _ui->background_end_scale->value();

  _ui->background_end_scale->setValue(
      std::max(value, _ui->background_begin_scale->value()));
}
