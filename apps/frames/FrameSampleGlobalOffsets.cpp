...
FrameSampleGlobalOffsets::create(ExperimentItem *experiment_item) {
  if (!_instance) {
...
FrameSampleGlobalOffsets::FrameSampleGlobalOffsets(
    ...
    : NSXQFrame(), _ui(new Ui::FrameSampleGlobalOffsets),
  ...
  const auto &sample_gonio =
      _experiment_item->experiment()->diffractometer()->sample().gonio();
  size_t n_axes = sample_gonio.nAxes();
  _ui->offsets->setRowCount(n_axes);
  for (size_t i = 0; i < n_axes; ++i) {
    const auto &axis = sample_gonio.axis(i);
    _ui->offsets->setItem(
        i, 0, new QTableWidgetItem(QString::fromStdString(axis.name())));
  }
...
void FrameSampleGlobalOffsets::fit() {
  ...
  const auto &sample =
      _experiment_item->experiment()->diffractometer()->sample();
