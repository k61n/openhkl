...
FrameStatistics *FrameStatistics::create(const nsx::PeakList &peaks,
...
FrameStatistics *FrameStatistics::Instance() { return _instance; }

FrameStatistics::FrameStatistics(const nsx::PeakList &peaks,
                                 const nsx::SpaceGroup &space_group)
    : NSXQFrame(), _ui(new Ui::FrameStatistics), _peaks(peaks),
      _space_group(space_group), _merged_data(space_group, true) {
  _ui->setupUi(this);
  ...
  shell_model->setHorizontalHeaderLabels(
      {"dmax", "dmin", "nobs", "nmerge", "redundancy", "Rmeas", "Rmeas(est.)",
       "Rmerge/Rsym", "Rmerge(est.)", "Rpim", "Rpim(est.)", "CChalf", "CC*"});

  QStandardItemModel *merged_peaks_model = new QStandardItemModel(7, 0, this);
  _ui->merged_peaks->setModel(merged_peaks_model);
  merged_peaks_model->setHorizontalHeaderLabels(
  ...
  connect(_ui->save_statistics, SIGNAL(clicked()), this,
          SLOT(saveStatistics()));
  connect(_ui->save_merged_peaks, SIGNAL(clicked()), this,
          SLOT(saveMergedPeaks()));
  connect(_ui->save_unmerged_peaks, SIGNAL(clicked()), this,
          SLOT(saveUnmergedPeaks()));
...
void FrameStatistics::plotStatistics(int column) {
  auto statistics_table_model =
      dynamic_cast<QStandardItemModel *>(_ui->statistics->model());
  ...
  for (auto i = 0; i < n_shells; ++i) {
    auto value = statistics_table_model->item(i, column)
                     ->data(Qt::DisplayRole)
                     .value<double>();
...
void FrameStatistics::saveToFullProf(QTableView *table) {
  ...
  double wavelength =
      _peaks[0]->data()->reader()->metadata().key<double>("wavelength");
...
void FrameStatistics::updateStatisticsTab() {
  ...
  for (int i = n_shells - 1; i >= 0; --i) {
    ...
    row.append(
        new QStandardItem(QString::number(merged_data_per_shell.totalSize())));
    row.append(new QStandardItem(
        QString::number(merged_data_per_shell.peaks().size())));
    row.append(
        new QStandardItem(QString::number(merged_data_per_shell.redundancy())));
