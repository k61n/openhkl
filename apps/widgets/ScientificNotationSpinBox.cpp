...
//
//! @file      apps/widgets/ScientificNotationSpinBox.cpp
//! @brief     Implements class ScientificNotationSpinBox
//!
//! @homepage  ###HOMEPAGE###
...
ScientificNotationSpinBox::ScientificNotationSpinBox(QWidget* parent) : QDoubleSpinBox(parent)
{
...
bool ScientificNotationSpinBox::isIntermediateValueHelper(qint64 num, qint64 min, qint64 max, qint64* match) const
{
    if (num >= min && num <= max) {
        if (match)
            *match = num;
