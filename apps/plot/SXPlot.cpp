...
SXPlot::SXPlot(QWidget *parent) : QCustomPlot(parent) {
  ...
  connect(this, SIGNAL(titleDoubleClick(QMouseEvent *, QCPPlotTitle *)), this,
  ...
  connect(this, SIGNAL(customContextMenuRequested(QPoint)), this,
          SLOT(setmenuRequested(QPoint)));
}

void SXPlot::update(PlottableGraphicsItem *item){Q_UNUSED(item)}
...
void SXPlot::titleDoubleClick(QMouseEvent *event, QCPPlotTitle *title) {
  ...
  bool ok;
  QString newTitle = QInputDialog::getText(this, "NSXTool",
                                           "New plot title:", QLineEdit::Normal,
                                           title->text(), &ok);
  if (ok) {
...
void SXPlot::legendDoubleClick(QCPLegend *legend, QCPAbstractLegendItem *item) {
            ...
            // padding of legend where there is no item, then item is 0)
  {
    ...
    bool ok;
    QString newName =
        QInputDialog::getText(this, "NSXTool", "New legent:", QLineEdit::Normal,
...
void SXPlot::exportToAscii() {
  ...
  for (int i = 0; i < ngraphs; ++i) {
    ...
    for (auto point : *data) {
      ...
      result(current++, 2 * i + 2) =
          0.5 * (point.valueErrorPlus + point.valueErrorMinus);
    }
  }
  file << result;
  file.close();
