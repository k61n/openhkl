...
GLTrackball::GLTrackball(int viewport_w, int viewport_h)
    : _w(viewport_w), _h(viewport_h) {
  reset();
  ...
  // prevent recalculation of this factor every time a generateRotationTo call
  // is issued.
  _rotationspeed = 0.2;
...
void GLTrackball::generateRotationTo(int a, int b) {
  ...
  // Create a quaternion which brings the lastpoint to new point through a
  // rotation
  Eigen::Quaterniond temp =
      Eigen::Quaterniond::FromTwoVectors(_lastpoint, newpoint);
  // Left multiply
...
Eigen::Vector3d GLTrackball::getModelCenter() const { return _modelCenter; }
