...
void GLWidget::resizeGL(int w, int h) {
  ...
  glLoadIdentity();
  glOrtho(-100.0 * _zoom, 100.0 * _zoom, -100.0 * _zoom, 100.0 * _zoom,
          -100.0 * _zoom, 100.0 * _zoom);
...
void GLWidget::mousePressEvent(QMouseEvent *event) {
  if (_picking) {
    ...
    glReadPixels(event->x(), viewport[3] - event->y(), 1, 1, GL_RGBA,
...
GLScene &GLWidget::getScene() { return _scene; }
