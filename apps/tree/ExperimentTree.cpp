...
ExperimentTree::ExperimentTree(QWidget *parent) : QTreeView(parent) {
  ...
  connect(this, SIGNAL(clicked(QModelIndex)), this,
          SLOT(onSingleClick(QModelIndex)));
}
...
void ExperimentTree::onCustomMenuRequested(const QPoint &point) {
  ...
  if (index == rootIndex()) {
    ...
    if (auto exp_item = dynamic_cast<ExperimentItem *>(item)) {
      ...
      connect(user_defined, triggered,
              [=]() { pitem->openUserDefinedUnitCellIndexerFrame(); });
      ...
      QAction *refine =
          menu->addAction("Refine lattice and instrument parameters");
      ...
      QAction *openSampleGlobalOffsets =
          menu->addAction("Sample goniometer global offsets");
      connect(openSampleGlobalOffsets, &QAction::triggered,
              [=]() { sitem->openSampleGlobalOffsetsFrame(); });
    } else if (DetectorItem *detector_item =
                   dynamic_cast<DetectorItem *>(item)) {
      ...
      connect(predict, triggered,
              [=]() { lib_item->incorporateCalculatedPeaks(); });
    } else if (UnitCellsItem *unit_cells_item =
                   dynamic_cast<UnitCellsItem *>(item)) {
      QAction *remove_unused_unit_cell =
          menu->addAction("Remove unused unit cells");
...
void ExperimentTree::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Delete) {
    ...
    while (it.hasPrevious()) {
      auto item =
          dynamic_cast<SessionModel *>(model())->itemFromIndex(it.previous());
      if (!item->parent()) {
