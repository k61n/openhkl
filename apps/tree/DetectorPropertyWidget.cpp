...
DetectorPropertyWidget::DetectorPropertyWidget(DetectorItem *caller,
                                               QWidget *parent)
    : QWidget(parent), _ui(new Ui::DetectorPropertyWidget),
      _detectorItem(caller) {
  _ui->setupUi(this);

  const auto *detector =
      _detectorItem->experiment()->diffractometer()->detector();
...
DetectorPropertyWidget::~DetectorPropertyWidget() { delete _ui; }
