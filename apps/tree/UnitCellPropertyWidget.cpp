...
UnitCellPropertyWidget::UnitCellPropertyWidget(UnitCellItem *unit_cell_item,
                                               QWidget *parent)
    : QWidget(parent), _unitCellItem(unit_cell_item),
  ...
  QCompleter *completer =
      new QCompleter(ui->spaceGroup->model(), ui->spaceGroup);
  ...
  connect(_unitCellItem->model(), SIGNAL(itemChanged(QStandardItem *)), this,
  ...
  connect(ui->spaceGroup, SIGNAL(currentIndexChanged(QString)), this,
          SLOT(setSpaceGroup(QString)));
  connect(ui->spaceGroup->completer(), SIGNAL(activated(const QString &)), this,
          SLOT(activateSpaceGroupCompletion(const QString &)));
  connect(ui->indexingTolerance, SIGNAL(valueChanged(double)), this,
          SLOT(setIndexingTolerance(double)));
  connect(ui->z, SIGNAL(valueChanged(int)), this, SLOT(setZValue(int)));
  connect(ui->chemicalFormula, SIGNAL(editingFinished()), this,
          SLOT(setChemicalFormula()));
}

UnitCellPropertyWidget::~UnitCellPropertyWidget() { delete ui; }
...
void UnitCellPropertyWidget::setLatticeParams() {
  ...
  try {
    _unitCellItem->data(Qt::UserRole)
        ...
        ->setParameters(a, b, c, alpha * nsx::deg, beta * nsx::deg,
                        gamma * nsx::deg);
...
void UnitCellPropertyWidget::setChemicalFormula() {
  ...
  try {
    _unitCellItem->data(Qt::UserRole)
        .value<nsx::sptrUnitCell>()
        ->setMaterial(std::unique_ptr<nsx::Material>(
            new nsx::Material(formula.toStdString())));
...
void UnitCellPropertyWidget::update(QStandardItem *item) {
  ...
  ui->spaceGroup->setCurrentText(
      QString::fromStdString(unit_cell->spaceGroup().symbol()));
