...
DialogSpaceGroup::DialogSpaceGroup(const nsx::PeakList &peaks, QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogSpaceGroup), _groups(), _cell(nullptr),
      _selectedGroup("") {
  ui->setupUi(this);
    ...
    if (!_cell) {
      ...
      if (_cell != current_peak_cell) {
        throw std::runtime_error(
            "Only one unit cell is supported at this time");
      }
    }
  }
...
DialogSpaceGroup::~DialogSpaceGroup() { delete ui; }

std::string DialogSpaceGroup::getSelectedGroup() { return _selectedGroup; }
...
void DialogSpaceGroup::evaluateSpaceGroups() {
  ...
  nsx::info() << "Evaluating " << symbols.size() << " space groups based on "
              << hkls.size() << " peaks";
...
void DialogSpaceGroup::buildTable() {
  ...
  for (auto &&item : _groups) {
    ...
    QStandardItem *col2 =
        new QStandardItem(QString::fromStdString(grp.bravaisTypeSymbol()));
