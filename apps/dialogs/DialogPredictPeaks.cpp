...
DialogPredictPeaks *
DialogPredictPeaks::create(ExperimentItem *experiment_item,
...
DialogPredictPeaks *DialogPredictPeaks::Instance() { return _instance; }

DialogPredictPeaks::DialogPredictPeaks(ExperimentItem *experiment_item,
                                       const nsx::UnitCellList &unit_cells,
                                       QWidget *parent)
    : QDialog(parent), _ui(new Ui::DialogPredictPeaks),
...
void DialogPredictPeaks::predictPeaks() {
  ...
  nsx::PeakInterpolation peak_interpolation =
      static_cast<nsx::PeakInterpolation>(interpolation);
  ...
  for (auto d : data) {
    ...
    nsx::info() << "Predicting peaks for numor " << ++current_numor << " of "
                << data.size();

    auto &&predicted =
        nsx::predictPeaks(*library, d, unit_cell, d_min, d_max, radius,
                          n_frames, min_neighbors, peak_interpolation);
  ...
  nsx::info() << "Completed  peak prediction. Added " << predicted_peaks.size()
              << " peaks";
