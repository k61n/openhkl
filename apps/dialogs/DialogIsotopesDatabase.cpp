...
DialogIsotopesDatabase::DialogIsotopesDatabase(QWidget *parent)
  ...
  for (const auto &isotope : isotopes) {
    ...
    for (const auto &prop : properties) {
      ...
      if (imgr->hasProperty(isotopeName, pName)) {
        ...
        case nsx::ChemicalPropertyType::Int: {
          item->setText(
              QString::number(imgr->property<int>(isotopeName, pName)));
          break;
        }
        case nsx::ChemicalPropertyType::Double: {
          auto value =
              imgr->property<double>(isotopeName, pName) / um->get(pUnit);
          item->setText(QString::number(value));
          break;
        }
        case nsx::ChemicalPropertyType::Complex: {
          auto value =
              imgr->property<std::complex<double>>(isotopeName, pName) /
              um->get(pUnit);
        ...
        case nsx::ChemicalPropertyType::Bool: {
          item->setText(
              QString::number(imgr->property<bool>(isotopeName, pName)));
          break;
        }
        default:
    ...
    model->setVerticalHeaderItem(
        isotopeCount++,
        new QStandardItem(QString::fromStdString(isotope.first)));
  }
...
DialogIsotopesDatabase::~DialogIsotopesDatabase() { delete ui; }
