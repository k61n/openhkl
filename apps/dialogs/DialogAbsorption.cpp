...
DialogAbsorption::DialogAbsorption(nsx::sptrExperiment experiment,
                                   QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogAbsorption), _experiment(experiment),
      _cscene(
          new CrystalScene(&experiment->diffractometer()->sample().shape())) {
  ui->setupUi(this);
  ...
  connect(ui->horizontalScrollBar, &QScrollBar::valueChanged, [=](int i) {
    _cscene->loadImage(QString::fromStdString(_imageList[i].second));
  });
  connect(ui->horizontalScrollBar, &QScrollBar::valueChanged, [=](int i) {
    _cscene->drawText("Phi: " + QString::number(_imageList[i].first));
    connect(ui->horizontalScrollBar, &QScrollBar::valueChanged,
            [=](int i) { _cscene->setRotationAngle(_imageList[i].first); });
  });

  //
  connect(ui->button_calibrateDistance, SIGNAL(pressed()), _cscene,
          SLOT(activateCalibrateDistance()));
  connect(ui->button_pickCenter, SIGNAL(pressed()), _cscene,
          SLOT(activatePickCenter()));
  connect(ui->button_pickingPoints, SIGNAL(pressed()), _cscene,
          SLOT(activatePickingPoints()));
  connect(ui->button_removingPoints, SIGNAL(pressed()), _cscene,
          SLOT(activateRemovingPoints()));
  connect(ui->button_triangulate, SIGNAL(pressed()), _cscene,
          SLOT(triangulate()));

  connect(_cscene, &CrystalScene::calibrateDistanceOK,
          [=]() { ui->button_pickCenter->setEnabled(true); });
  connect(_cscene, &CrystalScene::calibrateCenterOK,
          [=]() { ui->button_pickingPoints->setEnabled(true); });
  connect(_cscene, &CrystalScene::calibrateCenterOK,
          [=]() { ui->button_removingPoints->setEnabled(true); });
  connect(_cscene, &CrystalScene::calibrateCenterOK,
          [=]() { ui->button_triangulate->setEnabled(true); });
...
DialogAbsorption::~DialogAbsorption() { delete ui; }
  ...
  if (file.is_open()) {
    ...
    if (_instrumentName.compare(diffType) != 0) {
      QMessageBox::critical(this, tr("NSXTool"),
                            tr("Instrument name in video file does not match "
    ...
    if (numberAngles == sample_gonio.nAxes()) {
      QMessageBox::critical(this, tr("NSXTool"),
                            tr("Number of goniometer axes in video file does "
    ...
    for (std::size_t i = 0; i < numberAngles; ++i) {
      ...
      if (axis.name().compare(name) != 0) {
        QMessageBox::critical(this, tr("NSXTool"),
                              tr("Mismatch between axis names in video file "
...
void DialogAbsorption::on_button_openFile_pressed() {
  ...
  QString fileName = dialog.getOpenFileName(this, "Select Video file", "",
                                            tr("Video file (*.info)"));
