...
//
//! @file      apps/dialogs/DialogAbsorption.cpp
//! @brief     Implements class DialogAbsorption
//!
//! @homepage  ###HOMEPAGE###
    ...
    , _cscene(new CrystalScene(&experiment->diffractometer()->sample().shape()))
{
    ...
    connect(ui->horizontalScrollBar, &QScrollBar::valueChanged, [=](int i) {
        ...
        connect(ui->horizontalScrollBar, &QScrollBar::valueChanged, [=](int i) {
            _cscene->setRotationAngle(_imageList[i].first);
        });
    });

    //
    connect(
        ui->button_calibrateDistance, SIGNAL(pressed()), _cscene,
