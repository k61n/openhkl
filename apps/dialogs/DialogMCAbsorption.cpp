...
DialogMCAbsorption::DialogMCAbsorption(ExperimentItem *experiment_item,
                                       QWidget *parent)
    : QDialog(parent), ui(new Ui::DialogMCAbsorption),
...
DialogMCAbsorption::~DialogMCAbsorption() { delete ui; }
...
void DialogMCAbsorption::on_pushButton_run_pressed() {
  ...
  if (material == nullptr) {
    QMessageBox::critical(this, "NSXTOOL",
                          "No material defined for this crystal");
    return;
  }
  ...
  if (!hull.checkEulerConditions()) {
    QMessageBox::critical(this, "NSXTOOL",
                          "The sample shape (hull) is ill-defined");
    return;
  }
  ...
  for (auto &d : data) {
    ...
    ui->progressBar_MCStatus->setFormat(
        QString::fromStdString(d.second->filename()) + ": " +
    ...
    for (auto &p : peaks) {
      ...
      auto position =
          data->reader()->diffractometer()->detector()->pixelPosition(coord[0],
                                                                      coord[1]);
