...
DialogPeakFilter *DialogPeakFilter::create(ExperimentItem *experiment_item,
...
DialogPeakFilter *DialogPeakFilter::Instance() { return _instance; }

DialogPeakFilter::DialogPeakFilter(ExperimentItem *experiment_item,
                                   const nsx::PeakList &peaks, QWidget *parent)
    : QDialog(parent), _ui(new Ui::DialogPeakFilter),
      _experiment_item(experiment_item), _peaks(peaks) {
  _ui->setupUi(this);
  ...
  for (auto unit_cell : unit_cells) {
    _ui->unitCells->addItem(QString::fromStdString(unit_cell->name()),
                            QVariant::fromValue(unit_cell));
  }

  _peaks_model = new CollectedPeaksModel(
      _experiment_item->model(), _experiment_item->experiment(), _peaks);
  _ui->peaks->setModel(_peaks_model);

  connect(_ui->unitCells, SIGNAL(currentIndexChanged(int)), this,
          SLOT(slotUnitCellChanged(int)));
...
void DialogPeakFilter::slotUnitCellChanged(int index) {
  ...
  auto unit_cell =
      _ui->unitCells->itemData(index, Qt::UserRole).value<nsx::sptrUnitCell>();
...
void DialogPeakFilter::filterPeaks() {
  ...
  if (_ui->state->isChecked()) {
    filtered_peaks =
        peak_filter.selected(filtered_peaks, _ui->selected->isChecked());
    filtered_peaks =
        peak_filter.masked(filtered_peaks, _ui->masked->isChecked());
    filtered_peaks =
        peak_filter.predicted(filtered_peaks, _ui->predicted->isChecked());
  }
  ...
  if (_ui->indexedByUnitCell->isChecked()) {
    if (_ui->unitCells->count() > 0) {
      auto unit_cell =
          _ui->unitCells->itemData(_ui->unitCells->currentIndex(), Qt::UserRole)
              .value<nsx::sptrUnitCell>();
      filtered_peaks = peak_filter.indexed(filtered_peaks, *unit_cell,
                                           _ui->indexingTolerance->value());
    }
  }
...
void DialogPeakFilter::accept() {
  ...
  if (!filtered_peaks.empty()) {
    ...
    nsx::info() << "Applied peak filters on selected peaks. Remains "
                << filtered_peaks.size() << " out of " << _peaks.size()
                << " peaks";
  }
