...
static PeakFactors peakFactors(nsx::sptrPeak3D peak) {
  ...
  auto position =
      peak->data()->reader()->diffractometer()->detector()->pixelPosition(
          coord[0], coord[1]);
...
CollectedPeaksModel::CollectedPeaksModel(SessionModel *session,
                                         nsx::sptrExperiment experiment,
                                         QObject *parent)
    : QAbstractTableModel(parent), _experiment(std::move(experiment)),
      _peaks() {
  setSession(session);
}
...
SessionModel *CollectedPeaksModel::session() { return _session; }

void CollectedPeaksModel::setSession(SessionModel *session) {
  _session = session;
  connect(this, &CollectedPeaksModel::signalSelectedPeakChanged,
          [this](nsx::sptrPeak3D peak) {
  ...
  connect(_session, SIGNAL(signalUnitCellRemoved(nsx::sptrUnitCell)), this,
          SLOT(slotRemoveUnitCell(nsx::sptrUnitCell)));
}

void CollectedPeaksModel::slotRemoveUnitCell(
...
const nsx::PeakList &CollectedPeaksModel::peaks() const { return _peaks; }
...
QVariant CollectedPeaksModel::headerData(int section,
...
void CollectedPeaksModel::setUnitCell(const nsx::sptrUnitCell &unitCell,
...
nsx::sptrExperiment CollectedPeaksModel::experiment() { return _experiment; }
