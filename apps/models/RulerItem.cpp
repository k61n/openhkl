...
void RulerItem::setTipWidth(double w) { _hwidth = w / 2.0; }
...
void RulerItem::moveTipAt(const QPointF &pos) {
  ...
  // Set the three lines
  _base->setLine(_start.x() + sangle, _start.y() - cangle, _start.x() - sangle,
                 _start.y() + cangle);
  _body->setLine(_start.x(), _start.y(), pos.x(), pos.y());
  _tip->setLine(pos.x() + sangle, pos.y() - cangle, pos.x() - sangle,
                pos.y() + cangle);
  // Set up text
  std::ostringstream os;
  os << "(" << pos.x() << "," << pos.y() << ")";
  ...
  if (_displaydistance && _calibrated) {
    ...
    _bodytext->setPos(_start.x() + 0.5 * dd * cangle / _hwidth + 8,
...
void RulerItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                      QWidget *widget) {
  Q_UNUSED(widget);
  Q_UNUSED(option);
...
QGraphicsItemGroup *RulerItem::stampRuler(double distance, const QPointF &pos) {
  ...
  QGraphicsTextItem *text =
      new QGraphicsTextItem(QString::fromStdString(os.str()));
...
void RulerItem::activateDistanceDisplay() { _displaydistance = true; }

double RulerItem::getConversion() const { return _conversion; }
