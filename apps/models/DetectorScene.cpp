...
DetectorScene::DetectorScene(QObject *parent)
    : QGraphicsScene(parent), _currentData(nullptr), _currentFrameIndex(-1),
      _currentIntensity(10), _currentFrame(), _cursorMode(PIXEL), _mode(ZOOM),
      _zoomstart(0, 0), _zoomend(0, 0), _zoomrect(nullptr), _zoomStack(),
      _itemSelected(false), _image(nullptr), _masks(), _lastClickedGI(nullptr),
      _logarithmic(false), _drawIntegrationRegion(false),
      _colormap(new ColorMap()), _integrationRegion(nullptr), _session(nullptr),
      _selected_peak_gi(nullptr), _peak_graphics_items(),
      _selected_peak(nullptr) {}

SessionModel *DetectorScene::session() { return _session; }
...
void DetectorScene::setSession(SessionModel *session) {
  ...
  connect(_session, SIGNAL(signalEnabledPeakChanged(nsx::sptrPeak3D)), this,
          SLOT(slotChangeEnabledPeak(nsx::sptrPeak3D)));

  connect(_session, SIGNAL(signalMaskedPeaksChanged(const nsx::PeakList &)),
          this, SLOT(slotChangeMaskedPeaks(const nsx::PeakList &)));
}
      ...
      if (auto p = dynamic_cast<CutterGraphicsItem *>(_lastClickedGI)) {
        if (true) {
          ...
          emit updatePlot(p);
                     ...
                     dynamic_cast<PlottableGraphicsItem *>(_lastClickedGI)) {
        emit updatePlot(p);
        ...
        emit _session->signalMaskedPeaksChanged(peaks);
                     ...
                     dynamic_cast<EllipseMaskGraphicsItem *>(_lastClickedGI)) {
        auto it = findMask(p);
        if (it != _masks.end()) {
...
void DetectorScene::createToolTipText(QGraphicsSceneMouseEvent *event) {
  ...
  nsx::InstrumentState state =
      _currentData->interpolatedState(_currentFrameIndex);
  ...
  auto pos =
      _currentData->reader()->diffractometer()->detector()->pixelPosition(col,
                                                                          row);
  ...
  case MILLER_INDICES: {
    ...
    if (!experiment_item) {
      ...
      if (selected_unit_cell_item) {
        ...
        auto miller_indices =
            nsx::MillerIndex(q, *(selected_unit_cell_item->data(Qt::UserRole)
...
nsx::sptrDataSet DetectorScene::getData() { return _currentData; }
...
int DetectorScene::currentFrame() const { return _currentFrameIndex; }

void DetectorScene::updateMasks() { _lastClickedGI = nullptr; }
...
void DetectorScene::setLogarithmic(bool checked) { _logarithmic = checked; }
...
DetectorScene::findMask(QGraphicsItem *item) {
  return std::find_if(
      ...
      [item](const std::pair<QGraphicsItem *, nsx::IMask *> &x) {
        return x.first == item;
      });
}
