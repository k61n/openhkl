...
void DataItem::importData() {
  ...
  filenames =
      QFileDialog::getOpenFileNames(nullptr, "select numors", "", "", nullptr,
...
void DataItem::importRawData() {
  ...
  qfilenames =
      QFileDialog::getOpenFileNames(nullptr, "select raw data", "", "", nullptr,
                                    QFileDialog::Option::DontUseNativeDialog);
  ...
  try {
    ...
    nsx::error() << "reading numor:" << filenames[0].c_str()
                 << " reason not known:";
    return;
  }
...
nsx::DataList DataItem::selectedData() {
  ...
  for (int i = 0; i < rowCount(); ++i) {
    if (child(i)->checkState() == Qt::Checked) {
      if (auto numor_item = dynamic_cast<NumorItem *>(child(i))) {
        selectedNumors.push_back(
            numor_item->data(Qt::UserRole).value<nsx::sptrDataSet>());
      }
    }
  }
...
void DataItem::convertToHDF5() {
  ...
  std::unique_ptr<DialogHDF5Converter> dialog_ptr(
      new DialogHDF5Converter(selected_data));
