...
ExperimentItem::ExperimentItem(nsx::sptrExperiment experiment)
    : TreeItem(), _experiment(experiment) {
  ...
  _instr =
      new InstrumentItem(diff->name().c_str(), diff->source().name().c_str());
  appendRow(_instr);
...
InstrumentItem *ExperimentItem::instrumentItem() { return _instr; }

PeaksItem *ExperimentItem::peaksItem() { return _peaks; }

DataItem *ExperimentItem::dataItem() { return _data; }

LibraryItem *ExperimentItem::libraryItem() { return _library; }

UnitCellsItem *ExperimentItem::unitCellsItem() { return _unitCells; }
...
void ExperimentItem::writeLogFiles() {
  ...
  filtered_peaks =
      peak_filter.indexed(filtered_peaks, *cell, cell->indexingTolerance());

  FrameStatistics *frame =
      FrameStatistics::create(filtered_peaks, cell->spaceGroup());
