...
CrystalScene::CrystalScene(nsx::ConvexHull *hull, QWidget *parent)
    : QGraphicsScene(parent), pixmapitem(0), _ruler(0), _pin(0), _hull(hull),
      _text(nullptr) {
  distance = 1.0;
  _distancedialog = new DialogCalibrateDistance();
  connect(_distancedialog,
          SIGNAL(on_calibrateDistanceSpinBox_valueChanged(double)), this,
          SLOT(getDistance(double)));
  connect(_distancedialog, SIGNAL(on_calibrateDistanceButtonOK_accepted()),
          this, SLOT(stampScale()));
...
void CrystalScene::activateCalibrateDistance() { mode = calibrateDistance; }

void CrystalScene::activatePickCenter() { mode = pickCenter; }

void CrystalScene::activatePickingPoints() { mode = pickingPoint; }

void CrystalScene::activateRemovingPoints() { mode = removingPoint; }
...
void CrystalScene::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (mode == calibrateDistance) {
    ...
    if (CrystalNodeItem *temp2 = dynamic_cast<CrystalNodeItem *>(temp)) {
      ...
      _hull->removeVertex(
          Eigen::Vector3d(x * aspectratio, y * aspectratio, z * aspectratio),
          1e-3);
      removeItem(temp);
    }
  }
...
void CrystalScene::getDistance(double val) { distance = val; }
...
void CrystalScene::setRotationAngle(double angle) {
  ...
  double yc, zc;
  _pin->getCenter(yc, zc, _rotationAngle);
  for (QList<QGraphicsItem *>::iterator it = list.begin(); it != list.end();
       it++) {
