...
// special characters used in the dialog
static const QString g_angstrom(QChar(0x0212b));
static const QString g_sigma(QChar(0x03C3));
static const QString g_alpha(QChar(0x03B1));
static const QString g_beta(QChar(0x03B2));
static const QString g_gamma(QChar(0x03B3));
    ...
    _experiment(experiment)
{
    ...
    ui->label_amax->setText("Maximum cell dimension (" + g_angstrom + ")");
...
{
    double maxIsI = _peaks[size_t(peaknumber-1)]->getIOverSigmaI();
    ui->lineEdit_IOverSigmaI->setText("I/" + g_sigma + "I > " + QString::number(maxIsI));
}
...
void DialogFindUnitCell::on_pushButton_SearchUnitCells_clicked()
{
    ...
    for (unsigned int i = 0; i < tvects.size(); ++i) {
        for (unsigned int j = i+1; j < tvects.size(); ++j) {
            for (unsigned int k = j+1; k < tvects.size(); ++k) {
    ...
    for (unsigned int idx = 0; idx < new_solutions.size(); ++idx) {
        ...
        unsigned int nParameters = static_cast<unsigned int>(10 + sample->getNAxes() + detector->getNAxes());
        for (unsigned int i = 9; i < nParameters; ++i)
...
void DialogFindUnitCell::buildSolutionsTable()
{
    ...
    QStandardItemModel* model = new QStandardItemModel(int(_solutions.size()), 9, this);
    ...
    model->setHorizontalHeaderItem(3, new QStandardItem(g_alpha));
    model->setHorizontalHeaderItem(4, new QStandardItem(g_beta));
    model->setHorizontalHeaderItem(5, new QStandardItem(g_gamma));
    ...
    for (unsigned int i = 0; i < _solutions.size(); ++i) {
        ...
        model->setItem(int(i), 0, col1);
        model->setItem(int(i), 1, col2);
        model->setItem(int(i), 2, col3);
        model->setItem(int(i), 3, col4);
        model->setItem(int(i), 4, col5);
        model->setItem(int(i), 5, col6);
        model->setItem(int(i), 6, col7);
        model->setItem(int(i), 7, col8);
        model->setItem(int(i), 8, col9);
    }
...
{
    QMessageBox::information(this, tr("NSXTool"),tr("Solution set"));
    emit solutionAccepted(_solutions[size_t(i)].first);
}
