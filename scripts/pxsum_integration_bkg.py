...
for bkg in np.linspace(0.0, 1.0, num=6):
    ...
    integration_params.fixed_bkg_begin = 1.0 + bkg
    integration_params.fixed_bkg_end = 2.0 + bkg
    ...
    plt.plot(d, cchalf, label=f'Bkg = {bkg}', linewidth=0.5)

plt.legend()
plt.savefig("trypsin_pxsum_bkg.pdf")
