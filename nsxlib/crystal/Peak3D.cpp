...
void Peak3D::setPeakShape(SX::Geometry::IShape<double,3>* p)
{
    ...
    setSampleState(std::shared_ptr<ComponentState>(new ComponentState(_data->getSampleInterpolatedState(frame))));
    ComponentState detState = _data->getDetectorInterpolatedState(frame);
...
Eigen::RowVector3i Peak3D::getIntegerMillerIndices() const
{
    ...
    hkl << Utils::iround(_hkl[0]), Utils::iround(_hkl[1]), Utils::iround(_hkl[2]);
    return hkl;
}
...
void Peak3D::integrate()
{
    ...
    std::unique_ptr<IFrameIterator> it = _data->getIterator(static_cast<int>(idx));
...
double Peak3D::getSampleStepSize() const
{
    ...
    auto numFrames = _data->getNFrames();
    const auto& ss = _data->getSampleStates();
    auto numValues = ss[0].getValues().size();
...
void Peak3D::framewiseIntegrateBegin()
{
    ...
    //
    _state.data_start = static_cast<unsigned int>(Utils::ifloor(_state.lower[2]));
    _state.data_end = static_cast<unsigned int>(Utils::iceil(_state.upper[2]));

    _state.start_x=static_cast<unsigned int>(Utils::ifloor(_state.lower[0]));
    _state.end_x=static_cast<unsigned int>(Utils::iceil(_state.upper[0]));

    _state.start_y=static_cast<unsigned int>(Utils::ifloor(_state.lower[1]));
    _state.end_y=static_cast<unsigned int>(Utils::iceil(_state.upper[1]));
    ...
    _state.dx = static_cast<int>(_state.end_x - _state.start_x);
    _state.dy = static_cast<int>(_state.end_y - _state.start_y);
