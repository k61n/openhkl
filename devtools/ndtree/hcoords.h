...
template <typename T, std::size_t D> class HCoords : public ublas::bounded_vector<T, D> {
public:
    //! constructor
    HCoords();
...
template <typename T, std::size_t D> HCoords<T, D>::HCoords(const std::initializer_list<T>& other)
{
...
template <typename T, std::size_t D> HCoords<T, D>& HCoords<T, D>::operator=(HCoords<T, D>&& other)
{
    if (this != &other)
        this->assign_temporary(other);
