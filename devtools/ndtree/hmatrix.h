...
template <typename T, std::size_t D>
HMatrix<T, D>::HMatrix()
    : ublas::bounded_matrix<T, D, D>(ublas::zero_matrix<T>(D, D)) {}
...
template <typename T, std::size_t D>
void HMatrix<T, D>::scaling(
    const ublas::bounded_vector<T, D - 1> &scaleVector) {
  this->clear();
...
template <typename T, std::size_t D>
void HMatrix<T, D>::translation(
    const ublas::bounded_vector<T, D - 1> &transVector) {
