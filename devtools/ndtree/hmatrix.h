...
template <typename T, std::size_t D> class HMatrix : public ublas::bounded_matrix<T, D, D> {
public:
    //! constructor
    HMatrix();
...
template <typename T, std::size_t D> HMatrix<T, D>::HMatrix(const HMatrix& other)
{
    this->data() = other.data();
}

template <typename T, std::size_t D> HMatrix<T, D>::HMatrix(HMatrix&& other)
{
...
template <typename T, std::size_t D> HMatrix<T, D>& HMatrix<T, D>::operator=(HMatrix<T, D>&& other)
{
    if (this != &other)
        this->assign_temporary(other);
...
template <typename T, std::size_t D> void HMatrix<T, D>::scaling(T value)
{
    this->clear();
    for (std::size_t i = 0; i < D - 1; ++i)
...
template <typename T, std::size_t D> void HMatrix<T, D>::translation(T value)
{
    this->assign(ublas::identity_matrix<T>(D));
    for (std::size_t i = 0; i < D - 1; ++i)
