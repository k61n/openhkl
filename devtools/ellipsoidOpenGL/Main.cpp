...
public:
  EllipsoidGL(const vector3d &center, const vector3d &extents,
              const matrix3d &axis);
...
EllipsoidGL::EllipsoidGL(const vector3d &center, const vector3d &extents,
                         const matrix3d &axis)
    : Ellipsoid3D(center, extents, axis), _intercept(false) {
  _trans = this->getInverseTransformation().inverse();
}

bool EllipsoidGL::getIntercept() { return _intercept; }
...
void EllipsoidGL::setIntercept(bool intercept) { _intercept = intercept; }
...
int Data::getNumberOfFrames() { return _nFrames; }
...
blob3DCollection Data::getEllipsoids(double threshold, double confidence) {
  ...
  blob3DCollection blobs =
      findBlobs3D<int>(ptr, 256, 640, threshold, 5, 1000, confidence, 0);
  return blobs;
}
