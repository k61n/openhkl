...
void SessionModelDelegate::paint(
    QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    // if the underlying model is not a session model (should not happen) call the parent paint
    // method
    if (!session_model) {
        return QStyledItemDelegate::paint(painter, option, index);
    }

    auto unit_cells_item =
        dynamic_cast<UnitCellsItem*>(session_model->itemFromIndex(index.parent()));
    if (!unit_cells_item) {
        return QStyledItemDelegate::paint(painter, option, index);
    }
    ...
    opt->state |= QStyle::State_Enabled
        | (index.data(Qt::CheckStateRole).toBool() ? QStyle::State_On : QStyle::State_Off);

    style->drawControl(QStyle::CE_RadioButton, opt, painter);
}

bool SessionModelDelegate::editorEvent(
