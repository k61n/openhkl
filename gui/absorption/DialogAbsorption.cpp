...
void DialogAbsorption::readInfoFile(const std::string& filename)
{
    ...
    if (file.is_open()) {
        ...
        if (numberAngles == sample_gonio.nAxes()) {
            QMessageBox::critical(
                this, tr("NSXTool"),
                tr("Number of goniometer axes in video file does not match instrument definition"));
        }
        ...
        for (std::size_t i = 0; i < numberAngles; ++i) {
            ...
            if (axis.name().compare(name) != 0) {
                QMessageBox::critical(
                    this, tr("NSXTool"),
                    tr("Mismatch between axis names in video file and in instrument definition"));
            }
        }
