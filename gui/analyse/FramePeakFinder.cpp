...
FramePeakFinder* FramePeakFinder::create(
    ...
    , _peak_finders()
{
    ...
    connect(
        _ui->background_begin_scale, SIGNAL(valueChanged(double)), this,
        SLOT(changeBackgroundBegin()));
    connect(
        _ui->background_end_scale, SIGNAL(valueChanged(double)), this, SLOT(changeBackgroundEnd()));

    connect(
        _ui->convolution_kernels, SIGNAL(currentTextChanged(QString)), this,
    ...
    connect(
        _ui->actions, SIGNAL(clicked(QAbstractButton*)), this, SLOT(doActions(QAbstractButton*)));

    connect(
        _main_window->taskManagerModel(), SIGNAL(sendCompletedTask(std::shared_ptr<nsx::ITask>)),
        this, SLOT(onShowFoundPeaks(std::shared_ptr<nsx::ITask>)));
...
void FramePeakFinder::updateConvolutionParameters()
{
    ...
    disconnect(
        _ui->convolution_parameters, SIGNAL(cellChanged(int, int)), this,
...
void FramePeakFinder::preview()
{
    ...
    Eigen::MatrixXd convolved_frame =
        convolvedFrame(*data, selected_frame, convolver_type, convolver_parameters);
