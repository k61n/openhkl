...
PeaksTableModel::PeaksTableModel(const QString &name, nsx::sptrExperiment experiment, const nsx::PeakList &peaks, QObject *parent)
    : TableModel{name}
    , _experiment{experiment}
    , _peaks{peaks}
{}

PeaksTableModel::PeaksTableModel(const QString &name, nsx::sptrExperiment experiment, QObject *parent)
    : TableModel{name}
    , _experiment{experiment}
{}

void PeaksTableModel::slotRemoveUnitCell(
    const nsx::sptrUnitCell unit_cell) {
  Q_UNUSED(unit_cell)
...
QVariant PeaksTableModel::headerData(int section,
...
//    nsx::error() << "No unit cell defined for the peaks";
  ...
  std::sort(_peaks.begin(), _peaks.end(),
            [&](nsx::sptrPeak3D p1, nsx::sptrPeak3D p2) {
              nsx::MillerIndex miller_index1(p1->q(), *cell);
              nsx::MillerIndex miller_index2(p2->q(), *cell);
              return cell->spaceGroup().isEquivalent(miller_index1,
                                                     miller_index2);
            });
}

void PeaksTableModel::setUnitCell(const nsx::sptrUnitCell &unitCell,
...
void PeaksTableView::selectPeak(QModelIndex index) {
  ...
  PeaksTableModel *peaks_model =
      dynamic_cast<PeaksTableModel *>(model());
...
void PeaksTableView::keyPressEvent(QKeyEvent *event) {
  ...
  if (event->modifiers() == Qt::NoModifier) {
    ...
    } else if (key == Qt::Key_Up || key == Qt::Key_Backspace ||
               key == Qt::Key_PageDown) {
      if (current_index == previous_index) {
...
void PeaksTableView::contextMenuEvent(QContextMenuEvent *event) {
  ...
  connect(sortbyEquivalence, SIGNAL(triggered()), peaksModel,
          SLOT(sortEquivalents()));
  ...
  if (indexList.size()) {
    ...
    const auto &metadata =
        peaks[indexList[0].row()]->data()->reader()->metadata();
  ...
  connect(clearSelectedPeaks, SIGNAL(triggered()), this,
          SLOT(clearSelectedPeaks()));
  connect(selectAllPeaks, SIGNAL(triggered()), this, SLOT(selectAllPeaks()));
  connect(selectValidPeaks, SIGNAL(triggered()), this,
          SLOT(selectValidPeaks()));
  connect(selectUnindexedPeaks, SIGNAL(triggered()), this,
          SLOT(selectUnindexedPeaks()));
  connect(togglePeaksSelection, SIGNAL(triggered()), this,
          SLOT(togglePeaksSelection()));
}

void PeaksTableView::normalizeToMonitor() {
  bool ok;
  double factor = QInputDialog::getDouble(this, "Enter normalization factor",
                                          "", 1, 1, 100000000, 1, &ok);
