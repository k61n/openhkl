...
TEST_CASE("test/chemistry/TestIsotopeDatabase.cpp", "") {
    ...
    CHECK(
        imgr->property<std::complex<double>>("Hf[176]", "b_coherent") == std::complex<double>(6.61 * 1.0e-15, 0));
    ...
    CHECK(
        imgr->property<double>("H[1]", "molar_mass") == Approx(1.00782504 * um->get("g_per_mole")).epsilon(tolerance));
