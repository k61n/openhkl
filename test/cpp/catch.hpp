 ...
 */
#ifndef TEST_CPP_CATCH_HPP
#define TEST_CPP_CATCH_HPP
// start catch.hpp
...
#define INTERNAL_CATCH_TYPE_GEN                                                                    \
    ...
    {                                                                                              \
        return {};                                                                                 \
                                                                                                   ...
                                                                                                   \
    template <                                                                                     \
        template <typename...> class L1, typename... E1, template <typename...> class L2,          \
        typename... E2>                                                                            \
    ...
    {                                                                                              \
        return {};                                                                                 \
                                                                                                   ...
                                                                                                   \
    template <                                                                                     \
        template <typename...> class Container, template <typename...> class List,                 \
        typename... elems>                                                                         \
    ...
    {                                                                                              \
        return {};                                                                                 \
                                                                                                   ...
                                                                                                   \
    template <                                                                                     \
        template <typename...> class Final, template <typename...> class... Containers,            \
        typename... Types>                                                                         \
...
#define INTERNAL_CATCH_NTTP_REGISTER0(TestFunc, signature)                                         \
    ...
    {                                                                                              \
        Catch::AutoReg(                                                                            \
            Catch::makeTestInvoker(&TestFunc<Type>), CATCH_INTERNAL_LINEINFO, Catch::StringRef(),  \
            nameAndTags);                                                                          \
    }
...
#define INTERNAL_CATCH_NTTP_REGISTER(TestFunc, signature, ...)                                     \
    ...
    {                                                                                              \
        Catch::AutoReg(                                                                            \
            Catch::makeTestInvoker(&TestFunc<__VA_ARGS__>), CATCH_INTERNAL_LINEINFO,               \
...
#define INTERNAL_CATCH_NTTP_REGISTER_METHOD0(TestName, signature, ...)                             \
    ...
    {                                                                                              \
        Catch::AutoReg(                                                                            \
            Catch::makeTestInvoker(&TestName<Type>::test), CATCH_INTERNAL_LINEINFO, className,     \
            nameAndTags);                                                                          \
    }
...
#define INTERNAL_CATCH_NTTP_REGISTER_METHOD(TestName, signature, ...)                              \
    ...
    {                                                                                              \
        Catch::AutoReg(                                                                            \
            Catch::makeTestInvoker(&TestName<__VA_ARGS__>::test), CATCH_INTERNAL_LINEINFO,         \
...
struct AutoReg : NonCopyable {
    AutoReg(
        ITestInvoker* invoker, SourceLineInfo const& lineInfo, StringRef const& classOrMethod,
...
struct IResultCapture {
    ...
    virtual void handleExpr(
        AssertionInfo const& info, ITransientExpression const& expr,
        AssertionReaction& reaction) = 0;
    virtual void handleMessage(
        AssertionInfo const& info, ResultWas::OfType resultType, StringRef const& message,
        AssertionReaction& reaction) = 0;
    virtual void
...
public:
    AssertionHandler(
        StringRef const& macroName, SourceLineInfo const& lineInfo, StringRef capturedExpression,
        ResultDisposition::Flags resultDisposition);
    ~AssertionHandler()
    {
        if (!m_completed) {
...
public:
    Capturer(
        StringRef macroName, SourceLineInfo const& lineInfo, ResultWas::OfType resultType,
        StringRef names);
    ~Capturer();
...
struct TestCaseInfo {
    ...
    TestCaseInfo(
        std::string const& _name, std::string const& _className, std::string const& _description,
...
TestCase makeTestCase(
    ITestInvoker* testCase, std::string const& className, NameAndTags const& nameAndTags,
...
struct AssertionStats {
    AssertionStats(
        AssertionResult const& _assertionResult, std::vector<MessageInfo> const& _infoMessages,
...
struct SectionStats {
    SectionStats(
        SectionInfo const& _sectionInfo, Counts const& _assertions, double _durationInSeconds,
...
private:
    ...
    void reportExpr(
        AssertionInfo const& info, ResultWas::OfType resultType, ITransientExpression const* expr,
        bool negated);
...
{
    CATCH_ENFORCE(
        ulps >= 0, "Invalid ULP setting: " << ulps << '.' << " ULPs have to be non-negative.");
}
...
struct GeneratorTracker : TestCaseTracking::TrackerBase, IGeneratorTracker {
    ...
    GeneratorTracker(
        TestCaseTracking::NameAndLocation const& nameAndLocation, TrackerContext& ctx,
        ITracker* parent)
...
TestCase makeTestCase(
    ITestInvoker* _testCase, std::string const& _className, NameAndTags const& nameAndTags,
...
#define CATCH_REQUIRE_THROWS_MATCHES(expr, exceptionType, matcher)                                 \
    INTERNAL_CATCH_THROWS_MATCHES(                                                                 \
        "CATCH_REQUIRE_THROWS_MATCHES", exceptionType, Catch::ResultDisposition::Normal, matcher,  \
        expr)
#endif // CATCH_CONFIG_DISABLE_MATCHERS
...
#define CATCH_CHECK_THROWS_MATCHES(expr, exceptionType, matcher)                                   \
    INTERNAL_CATCH_THROWS_MATCHES(                                                                 \
        "CATCH_CHECK_THROWS_MATCHES", exceptionType, Catch::ResultDisposition::ContinueOnFailure,  \
        matcher, expr)
#endif // CATCH_CONFIG_DISABLE_MATCHERS
...
#define CATCH_FAIL(...)                                                                            \
    INTERNAL_CATCH_MSG(                                                                            \
        "CATCH_FAIL", Catch::ResultWas::ExplicitFailure, Catch::ResultDisposition::Normal,         \
        __VA_ARGS__)
...
#define CATCH_SUCCEED(...)                                                                         \
    INTERNAL_CATCH_MSG(                                                                            \
        "CATCH_SUCCEED", Catch::ResultWas::Ok, Catch::ResultDisposition::ContinueOnFailure,        \
        __VA_ARGS__)
...
#define REQUIRE_FALSE(...)                                                                         \
    INTERNAL_CATCH_TEST(                                                                           \
        "REQUIRE_FALSE", Catch::ResultDisposition::Normal | Catch::ResultDisposition::FalseTest,   \
        __VA_ARGS__)
...
#define CHECK_THROWS_MATCHES(expr, exceptionType, matcher)                                         \
    INTERNAL_CATCH_THROWS_MATCHES(                                                                 \
        "CHECK_THROWS_MATCHES", exceptionType, Catch::ResultDisposition::ContinueOnFailure,        \
        matcher, expr)
#endif // CATCH_CONFIG_DISABLE_MATCHERS
...
#define FAIL_CHECK(...)                                                                            \
    INTERNAL_CATCH_MSG(                                                                            \
        "FAIL_CHECK", Catch::ResultWas::ExplicitFailure,                                           \
        Catch::ResultDisposition::ContinueOnFailure, __VA_ARGS__)
#define SUCCEED(...)                                                                               \
    INTERNAL_CATCH_MSG(                                                                            \
