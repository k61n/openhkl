...
//
//! @file      test/cpp/data/TestRadialConvolver.cpp
//! @brief     Test ...
//!
...
TEST_CASE("test/data/TestRadialConvolver.cpp", "")
{
    ...
    std::map<std::string, double> convolver_params = {{"r_in", 0}, {"r_out", 5}};
    ohkl::RadialConvolver convolver(convolver_params);
    ...
    CHECK(maxval == Approx(570.5238095238).epsilon(eps));
    CHECK(minval == Approx(-0.0538302277).epsilon(eps));
    ...
    cv::imwrite("radial.jpg", cv_image_8u);
