...
TEST_CASE("test/data/TestAnnularImageFilter.cpp", "")
{
    ...
    ohkl::ImageFilter* ann_filter = factory.create("Annular", params);
    ann_filter->setImage(image);
    ann_filter->filter();
    cv::Mat filtered_image = ann_filter->cvFilteredImage();
    ...
    enh_ann_filter->setImage(image);
    enh_ann_filter->filter();
    filtered_image = enh_ann_filter->cvFilteredImage();
