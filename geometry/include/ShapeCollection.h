...
public:
	...
	typedef std::vector<IPShape<T,D>*> shapeVect;
	...
	shapeVect where_is(const std::initializer_list<T>& point) const;
...
template<typename T, uint D>
typename ShapeCollection<T,D>::shapeVect ShapeCollection<T,D>::where_is(const std::initializer_list<T>& point) const
{
