...
template<typename T,uint D=3> bool collideOBBOBB(const OBB<T,3>& a, const OBB<T,3>& b)
{
	...
	{
		R0=eiga[i];
		R1=eigb[0]*Cabs.block(i,0,1,D);
		R=std::abs(ra.block(0,i,D,1)*diff);
		if (R<=R0+R1)
			return true;
	}
	...
	for (unsigned int i=0;i<D;++i)
	{
		R0=eiga[0]*Cabs.block(i,0,1,D);
		R1=eigb[i];
		R=std::abs(rb.block(0,i,D,1)*diff);
		if (R<=R0+R1)
			return true;
	}

	double A0D(ra.block(0,0,D,1)*diff);
	double A1D(ra.block(0,0,D,1)*diff);
	double A2D(ra.block(0,0,D,1)*diff);
