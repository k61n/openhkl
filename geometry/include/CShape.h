...
#ifndef NSXTOOL_CSHAPE_H_
#define NSXTOOL_CSHAPE_H_
...
public:
	...
	//! return whether or not a given point is inside one of the primitive shape the composite shape is made of
...
template<typename T, uint D>
CShape<T,D>::CShape() : AABB<T,D>()
{
	_shapes.reserve(10);
}

template<typename T,uint D>
void CShape<T,D>::add_shape(IPShape<T,D>* shape)
{
	_shapes.push_back(shape);
}

template<typename T,uint D>
int CShape<T,D>::n_shapes() const
{
	return _shapes.size();
}

template<typename T, uint D>
bool CShape<T,D>::is_inside(const std::initializer_list<T>& point) const
{
...
template<typename T, uint D>
std::vector<IPShape<T,D>*> CShape<T,D>::where_is(const std::initializer_list<T>& point) const
{
