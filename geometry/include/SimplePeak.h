...
class SimplePeak : public IROI<TYPE,DATATYPE,DIM>
{
public:
...
template<typename TYPE, typename DATATYPE, uint DIM>
SimplePeak<TYPE,DATATYPE,DIM>::SimplePeak() : IROI<TYPE,DATATYPE,DIM>()
{

}

template<typename TYPE, typename DATATYPE, uint DIM>
void SimplePeak<TYPE,DATATYPE,DIM>::add_signal(IPShape<TYPE,DIM>* peak)
{
	if (!(this->_shapes.empty()))
		throw("SimplePeak:the signal must be the first region to be inserted");

	this->add_shape(peak);
}

template<typename TYPE, typename DATATYPE, uint DIM>
void SimplePeak<TYPE,DATATYPE,DIM>::add_background(IPShape<TYPE,DIM>* bg)
{
	if (this->_shapes.size()!=1)
		throw("SimplePeak:the background must be inserted after the signal");

	this->add_shape(bg);
}

template<typename TYPE, typename DATATYPE, uint DIM>
bool SimplePeak<TYPE,DATATYPE,DIM>::is_inside(const std::initializer_list<TYPE>& point) const
{
	if (this->_shapes.size()!=2)
		throw("SimplePeak:the peak is not fully defined");
...
template<typename TYPE, typename DATATYPE, uint DIM>
bool SimplePeak<TYPE,DATATYPE,DIM>::is_inside_signal(const std::initializer_list<TYPE>& point) const
{
	if (this->_shapes.size()!=2)
		throw("SimplePeak:the peak is not fully defined");
...
template<typename TYPE, typename DATATYPE, uint DIM>
bool SimplePeak<TYPE,DATATYPE,DIM>::is_inside_background(const std::initializer_list<TYPE>& point) const
{
	if (this->_shapes.size()!=2)
		throw("SimplePeak:the peak is not fully defined");
